...
// ─── self-heal: unresumable stale session → fresh respawn ────────────
//
// Production bug (shumorobot general chat, 2026-06-01→03): the chat's
// persisted claude_session_id was minted 2026-05-27. Every message spawned
// `claude --resume <week-old-id>`; claude rendered the aged-session
// "Resuming the full session… Resume from summary?" dialog, the startup-gate
// confirmed it, the /compact resume exited code 0, and the gate reported
// TMUX_SESSION_GONE. polygram surfaced a scary "tmux session disappeared"
// error and gave up — so the chat sat dead for DAYS, re-resuming the same
// dead id on every retry. Fix: on a startup-gate death of a --resume spawn,
// clear the stale id and respawn ONCE with a fresh session (no aged-session
// dialog → clean start), exactly like the Music topic recovered when its
// cwd change minted a fresh session.
//
// Red→green: before the fix, start() rethrew on the first TMUX_SESSION_GONE,
// so spawn() was called ONCE (only `--resume`) and the assert on a fresh
// second spawn failed. After: spawn() is called twice, the retry dropping
// `--resume` for a fresh `--session-id`.
test('self-heal: --resume startup death (TMUX_SESSION_GONE) clears stale id + respawns fresh once', async () => {
  const os = require('node:os');
  const path = require('node:path');
  const STALE = 'stale-week-old-sid-0001';
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'cli-selfheal-'));
  // canResume=true requires the resume JSONL to exist at the cwd-mangled
  // projects path (lib/process/cli-process.js:566) — otherwise the first
  // attempt would already start fresh and there's no poisoned resume to heal.
  const projDir = path.join(os.homedir(), '.claude', 'projects', cwd.replace(/\//g, '-'));
  fs.mkdirSync(projDir, { recursive: true });
  const jsonl = path.join(projDir, `${STALE}.jsonl`);
  fs.writeFileSync(jsonl, '{}\n');

  const spawnArgs = [];
  const runner = {
    spawn: async (o) => { spawnArgs.push(o.args); },
    killSession: async () => {},
    sendControl: async () => {},
    // Pane "gone" → runStartupGate throws TMUX_SESSION_GONE on the first
    // poll, for BOTH the resume attempt and the fresh retry.
    captureWide: async (name) => { throw new Error(`can't find pane: ${name}`); },
  };
  const cp = new CliProcess({
    sessionKey: 'sess-selfheal',
    chatId: 'chat-selfheal',
    threadId: null,
    label: 'test-selfheal',
    tmuxRunner: runner,
    botName: 'testbot',
  ...
  try {
    // Both attempts die at the gate → after the one-shot fresh retry, start()
    // still rejects. The VALUE is in which spawns happened along the way.
    await assert.rejects(
      cp.start({ existingSessionId: STALE, cwd }),
      /TMUX_SESSION_GONE|tmux session disappeared/,
    );
  } finally {
    fs.rmSync(projDir, { recursive: true, force: true });
    fs.rmSync(cwd, { recursive: true, force: true });
    await cp.kill('test').catch(() => {});
  }

  assert.equal(spawnArgs.length, 2, 'must retry exactly once: spawn called twice (resume, then fresh)');
  assert.ok(
    spawnArgs[0].includes('--resume') && spawnArgs[0].includes(STALE),
    'first attempt must resume the stale session id',
  );
  assert.ok(!spawnArgs[1].includes('--resume'), 'retry must NOT --resume the dead session');
  assert.ok(spawnArgs[1].includes('--session-id'), 'retry must start a fresh --session-id');
});

// Guard: a FRESH spawn (no existingSessionId) that dies must NOT retry —
// only poisoned resumes self-heal; a genuine fresh failure throws once.
test('self-heal: fresh-spawn startup death does NOT retry (no infinite respawn)', async () => {
  const spawnArgs = [];
  const runner = {
    spawn: async (o) => { spawnArgs.push(o.args); },
    killSession: async () => {},
  ...
  const cp = new CliProcess({
    sessionKey: 'sess-fresh', chatId: 'chat-fresh', threadId: null,
    label: 'test-fresh', tmuxRunner: runner, botName: 'testbot',
    claudeBin: '/usr/bin/true', toolDispatcher: async () => ({ ok: true }),
    logger: quietLogger, handshakeTimeoutMs: 2000,
  });
  try {
    await assert.rejects(cp.start({ cwd: '/tmp' }), /TMUX_SESSION_GONE|tmux session disappeared/);
  } finally {
    await cp.kill('test').catch(() => {});
  }
  assert.equal(spawnArgs.length, 1, 'fresh-spawn failure must NOT retry');
});
