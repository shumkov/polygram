 ...
 *     interrupted work.
 *   - CLEAN (cleanShutdown=true): a deliberate restart. Exact Codex evidence
 *     that proves a request never crossed the provider boundary stays eligible
 *     for its dedicated recoverer. Other pending candidates are skipped, with
 *     one visibility notice per chat/topic for announceable legacy work.
 *
...
function classifyReplay({
  ...
  for (const candidate of pending) {
    ...
    if (disposition.action === 'legacy') {
      ...
      if (!cleanShutdown || disposition.cleanRestartSafe === true) {
        recoverCodex.push(candidate);
      } else {
        restartSkippedCodex.push(candidate);
      }
  ...
  // Deliberate restart: preserve exact clean-safe Codex recovery, skip the
  // remaining pending work, and group the ANNOUNCEABLE skipped rows per
  // (chat, thread) for one visibility notice each
  ...
  const restartSkipped = [...legacy, ...restartSkippedCodex];
 ...
 *   exact-runtime Codex re-dispatch
