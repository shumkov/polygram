 ...
 * with {timedout} (never resolved-but-hung). A failed Telegram send is also
...
function createQuestionHandlers({
  ...
  function finalize(row, result, status = 'answered') {
    try {
      answerQuestion?.(row.session_key, row.tool_call_id, result);
    } catch (e) {
  ...
  return { renderAsk, handleQuestionCallback, tryConsumeAsAnswer, expireQuestion };
}
