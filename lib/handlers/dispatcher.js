...
function createDispatcher({
  ...
  const activeHandlerTargets = new Map();
  ...
  function dispatchHandleMessage(sessionKey, chatId, msg, bot) {
    const targetOwner = {};
    activeHandlerTargets.set(targetOwner, Object.freeze({
    ...
    const task = handleMessage(sessionKey, chatId, msg, bot).catch(async (err) => {
      ...
      activeHandlerTargets.delete(targetOwner);
  ...
  return {
    ...
    getActiveHandlerTargets: () => [...activeHandlerTargets.values()],
    awaitSettlement,
    trackTask,
  };
}
