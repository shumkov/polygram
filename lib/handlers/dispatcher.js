...
function createDispatcher({
  ...
  // rc.51: shared pipeline for agent-authored replies — passed
  // through to processAndDeliverAgentText so the auto-resume reply
  // gets parseResponse + sanitize (matching the bot-reply-stream
  // path). Pre-rc.51 the auto-resume reply called deliverReplies
  // directly with raw text → [sticker:NAME] would have shown as
  // literal text and canned strings would have leaked, latent until
  // an auto-resume turn happened to emit either. Both optional with
  // pre-rc.51 raw-delivery fallback for back-compat.
  parseResponse = null,
  ...
  async function attemptAutoResume(sessionKey, chatId, originalMsg, bot) {
    ...
    // 4. Send the continuation reply through the shared agent-text
    //    pipeline (rc.51) — gives auto-resume replies the same
    //    parseResponse + sanitize protections as bot-reply-stream
    //    and the rc.50-fixed autonomous-wakeup path. Pre-rc.51 this
    //    went straight to deliverReplies with the raw text, so a
    //    `[sticker:NAME]` tag would have shown as literal text and
    //    a canned-string leak would have reached the user.
    const haveFullPipeline = (typeof parseResponse === 'function')
      && (typeof sanitizeAssistantReply === 'function');
    const deliverLogger = { error: (m) => logger.error?.(`[${sessionKey}] auto-resume deliver: ${m}`) };
    ...
    if (haveFullPipeline) {
      ...
      // Pre-rc.51 raw-delivery fallback (back-compat for test harnesses
      // that don't wire parseResponse / sanitizeAssistantReply).
