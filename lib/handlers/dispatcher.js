...
function createDispatcher({
  ...
  handleMessage,                  // async (sessionKey, chatId, msg, bot, dispatchContext)
  awaitSessionRecovery = () => Promise.resolve(),
  ...
  function dispatchHandleMessage(
    sessionKey,
    chatId,
    msg,
    bot,
    ...
    const task = Promise.resolve()
      .then(() => awaitSessionRecovery(
        sessionKey,
        dispatchContext?.recoveryReceipt ?? null,
      ))
      .then(() => handleMessage(
        sessionKey,
        chatId,
      ...
      const cleanReplayProcessDrift = (
        dispatchContext?.cleanReplay != null
        && err.code === 'CODEX_CLEAN_REPLAY_PROCESS_CHANGED'
      );
      ...
      logEvent('handler-error', {
        ...
        // A spawn failure's stderr is process output, not a field: its size
        // says whether the child said anything at all, which is the signal
        // that distinguishes "tmux refused" from "tmux said nothing".
        stderr_len: typeof err.stderr === 'string' ? err.stderr.length : undefined,
      ...
      if (err.code === 'SESSION_PROCESS_LOST') {
        try {
          ...
          logEvent('session-reset-after-process-loss-failed', {
            chat_id: chatId, session_key: sessionKey, msg_id: msg?.message_id,
            error_class: resetErr.name || 'Error',
          });
        }
      }
          ...
          return attemptAutoResume(sessionKey, chatId, msg, bot, {
            ...
            .catch((resumeErr) => {
              ...
              logEvent('auto-resume-failed', {
                chat_id: chatId, session_key: sessionKey, msg_id: msg.message_id,
                error_class: resumeErr?.name || 'Error',
              });
