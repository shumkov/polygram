...
function createHandleAbort({
  ...
  // Narrow async hook: retire the question state for a session whose process
  // is being stopped. Kept as a callback so this handler never learns about
  // the question store.
  ...
  return async function handleAbortIfRequested(msg, chatId, chatConfig, cleanText) {
    ...
    if (proc?.backend === 'codex') {
      ...
      // Only after the stop is proven, and only ever cleanup: Codex cannot
      // raise an interactive question, so this closes a row a Claude turn left
      // behind on a session that has since moved to Codex. An unverified stop
      // changes nothing, exactly as before.
      if (stopVerified) await retireQuestionSession?.(sessionKey);
      ...
      logEvent('abort-requested', {
        ...
        // The stop phrasing is the user's own message; its size is the signal.
    ...
    // The stopped turn owned any open question; retire its state with the same
    ...
    logEvent('abort-requested', {
      ...
      busy_probe: busyProbe ? {
        ...
        // The pane tail is live TUI transcript; only whether one was
        // captured is recorded, never its content.
      } : null,
      // The stop phrasing is the user's own message; its length is enough to
      // tell a bare "stop" from a sentence that happened to contain one.
      text_len: cleanText.length,
    });
