...
function createFormatConfigInfoText({
  ...
  function formatWithRuntimeView(
    ...
    const codex = isCodexRuntimeView(runtimeView);
    // The selected Codex model decides which reasoning efforts the help section
    // may list — per-topic overrides win over chat-level, mirroring the spawn
    // path. Nothing else about the configuration is stated here: the buttons
    // under the card carry it, each with its own ✓.
    const model = configuredRuntimeValue(
      chatConfig,
      topicConfig,
      runtimeView,
