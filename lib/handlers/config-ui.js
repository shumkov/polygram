...
const CODEX_MODEL_OPTIONS = Object.freeze([
  Object.freeze({ model: 'gpt-5.6-sol', label: 'Sol' }),
  Object.freeze({ model: 'gpt-5.6-terra', label: 'Terra' }),
  Object.freeze({ model: 'gpt-5.6-luna', label: 'Luna' }),
]);
...
// it has no button. Codex is filtered below unless enabled for the current
// scope. A chat configured on sdk simply shows no ✓ on this row.
...
function getCodexCatalogModels(runtimeView) {
  ...
  for (const entry of runtimeView.models) {
    if (entry && typeof entry.model === 'string' && entry.model.length > 0) {
      if (!catalogByModel.has(entry.model)) {
        catalogByModel.set(entry.model, entry);
      }
    }
  }
  return CODEX_MODEL_OPTIONS.flatMap(({ model }) => (
    catalogByModel.has(model) ? [catalogByModel.get(model)] : []
  ));
}

function codexModelLabel(model) {
  return CODEX_MODEL_OPTIONS.find((entry) => entry.model === model)?.label
    ?? model;
}

function hasCodexCatalogModels(runtimeView) {
  return isCodexRuntimeView(runtimeView)
    && Array.isArray(runtimeView.models)
    && runtimeView.models.some(
...
function buildConfigKeyboard(
  ...
  if (show === 'all') {
    ...
    rows.push(RUNTIME_OPTIONS
      .filter(({ backend }) => (
        backend !== 'codex' || runtimeView?.codexEnabled === true
      ))
      .map(({ backend, label }) => ({
  ...
  if ((show === 'model' || show === 'all') && modelOptions.length > 0) {
    rows.push(modelOptions.map((entry) => {
      ...
      if (codex) {
        label = codexModelLabel(entry.model);
      } else if (
  ...
  function formatWithRuntimeView(
    ...
    const modelHelp = codex
      ? codexModels.length > 0 ? [
        ...
        '**Models**',
        ...codexModels.map(
          (entry) => `• **${codexModelLabel(entry.model)}** — ${entry.model}`,
        ),
      ].join('\n') : [
        '',
        '**Models**',
        hasCodexCatalogModels(runtimeView)
    ...
    const effortHelp = codex
      ? codexEfforts.length > 0 ? [
        '',
        `**Effort for ${codexModelLabel(codexModel)}**`,
        codexEfforts.map((value) => `**${value}**`).join(' · '),
        'Changes apply on the next turn; an active turn is unchanged.',
      ].join('\n') : [
        '',
        '**Effort**',
      ...
      : [
        ...
        'Higher values allow more reasoning when needed.',
      ].join('\n');
    ...
    let body = '⚙️ Settings';
    if (codex) {
      body += '\n\n_Codex beta: no command network, web search, product MCP tools, '
        + 'or interactive approvals. Detached/background servers are unsupported '
        + 'and may survive hard runtime loss. '
        + 'Native goals are currently disabled._';
    }
