...
const RUNTIME_OPTIONS = Object.freeze([
  Object.freeze({ backend: 'sdk', label: 'Claude SDK' }),
  Object.freeze({ backend: 'cli', label: 'Claude CLI' }),
...
function runtimeSelectionSource(runtimeView) {
  const source = runtimeView?.selectionSource
    ?? runtimeView?.source
    ?? runtimeView?.sources?.pm;
  return typeof source === 'string' && source.length > 0 ? source : null;
}

function formatRuntimeLine(runtimeView) {
  const backend = canonicalRuntimeBackend(runtimeView);
  const label = backend === 'codex'
    ? 'Codex app-server'
    : backend === 'cli' ? 'Claude CLI' : 'Claude SDK';
  const source = runtimeSelectionSource(runtimeView);
  return `Runtime: ${label}${source ? ` (source: ${source})` : ''}`;
}
...
function createFormatConfigInfoText({
  ...
  function formatWithRuntimeView(
    ...
    const richTextLine = richText
      ...
      : 'Rich text: off';
    const runtimeLine = formatRuntimeLine(runtimeView);
    let head;
    if (codex) {
      const desired = runtimeView.desiredSettings ?? { model, effort };
      const desiredPair = formatSettingsPair(desired)
        ?? 'incomplete Codex selection';
