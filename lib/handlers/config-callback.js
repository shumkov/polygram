...
const { classify } = require('../error/classify');
const {
...
function codexSelectionErrorMessage(error) {
  if (
    error?.code === 'CODEX_SCOPE_DISABLED'
    || error?.code === 'CODEX_RUNTIME_SELECTION_INCOMPLETE'
  ) {
    return classify(error).userMessage;
  }
  return 'Codex is unavailable; check login and runtime diagnostics';
}
  ...
  return async function handleConfigCallback(ctx) {
    ...
    try {
      if (setting === 'runtime') {
        ...
        if (
          value === 'codex'
          && !resolveCodexEnabled(config, chatId, callbackThreadIdEarly)
        ) {
          releaseConfigIntent();
          await ctx.answerCallbackQuery({
            text: codexSelectionErrorMessage({ code: 'CODEX_SCOPE_DISABLED' }),
            show_alert: true,
          }).catch(() => {});
          return;
        }
        ...
        if (value === 'codex') {
          ...
          try {
            ...
            releaseConfigIntent();
            await ctx.answerCallbackQuery({
              text: codexSelectionErrorMessage(error),
              show_alert: true,
            }).catch(() => {});
            return;
          }
      ...
      // with it. Record WHICH ones, by id, so a config that moved on its own
      // is still explicable afterwards — the values they held are
      // configuration content and the ids answer the question without them.
      // Logged only once the sweep is persisted — an event for a rolled-back
      ...
      const logTopicRichTextSweep = (cleared) => {
        ...
        dbWrite(() => {
          db.logEvent('config-topic-override-swept', {
            chat_id: chatId,
            field: configField,
            topic_ids: cleared.map(({ threadId }) => String(threadId)),
