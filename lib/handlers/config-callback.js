...
function createHandleConfigCallback({
  ...
  return async function handleConfigCallback(ctx) {
    ...
    try {
      ...
      // A chat-level rich-text write is shadowed by any topic-level override
      // the pre-fix behavior left behind (delivery resolves topic first), so
      // clearing it is part of writing at chat level, not a separate cleanup.
      const shadowingTopic = (
        isRichText
        && writeThreadId === null
        && callbackThreadIdEarly != null
      ) ? chatConfig.topics?.[String(callbackThreadIdEarly)] : null;
      const shadowSnapshot = (
        shadowingTopic
        && typeof shadowingTopic === 'object'
        && Object.hasOwn(shadowingTopic, configField)
      ) ? { scope: shadowingTopic, value: shadowingTopic[configField] } : null;

      writeScope[configField] = parsedValue;
      if (shadowSnapshot) delete shadowSnapshot.scope[configField];
      if (effortAdjustment) {
      ...
      try {
        ...
        if (shadowSnapshot) shadowSnapshot.scope[configField] = shadowSnapshot.value;
        logger.error?.(
