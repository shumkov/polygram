 ...
 * no provider write occurred. A deploy-owned pre-write cancellation is safe
 * only with exact clean-restart provenance; other cancellations remain
 * cancelled. Every ambiguous state fails closed because re-submission could
 * duplicate text or tool effects.
 *
...
function classifyCodexRecoveryEvidence(evidence = {}) {
  const structured = (
    ...
    || evidence?.reservation != null
    || evidence?.attempt != null
    || evidence?.linkedInput != null
    || evidence?.targetAttempt != null
    || evidence?.cancellationProof != null
  );
  if (structured) {
    const selection = evidence?.selection;
    const reservation = evidence?.reservation;
    const attempt = evidence?.attempt;
    const linkedInput = evidence?.linkedInput;
    const targetAttempt = evidence?.targetAttempt;
    const cancellationProof = evidence?.cancellationProof;
    const exactSelection = (
      evidence?.provider === 'codex'
      && selection?.provider === 'codex'
      && typeof selection.sessionKey === 'string'
      && selection.sessionKey.length > 0
      && Number.isSafeInteger(selection.selectedTs)
      && selection.selectedTs >= 0
    );
    const exactReservation = (
      typeof reservation?.reservationId === 'string'
      && reservation.reservationId.length > 0
      && typeof reservation?.generationId === 'string'
      && reservation.generationId.length > 0
      && ['reserved', 'queue-authorized'].includes(reservation.state)
      && reservation.steerAttemptId == null
      && reservation.targetAttemptId == null
      && reservation.settledTs == null
    );
    const exactAttempt = (
      typeof attempt?.attemptId === 'string'
      && attempt.attemptId.length > 0
      && typeof attempt?.generationId === 'string'
      && attempt.generationId.length > 0
      && attempt.deliveryState === 'prepared'
      && attempt.turnId == null
      && attempt.terminalStatus == null
    );
    const noLinkedEvidence = linkedInput == null && targetAttempt == null;
    const matchingGeneration = (
      reservation == null
      || attempt == null
      || reservation.generationId === attempt.generationId
    );
    const exactOptionalQueueReservation = (
      reservation == null
      || (exactReservation && reservation.state === 'queue-authorized')
    );
    ...
    if (
      ...
      return {
        action: 'recover',
        reason: 'selection-proven-not-dispatched',
        cleanRestartSafe: true,
      };
    }
    if (
      evidence.kind === 'dispatch-reservation'
      && exactReservation
      && attempt == null
      && cancellationProof == null
    ) {
      return {
        action: 'recover',
        reason: 'reservation-proven-not-dispatched',
        cleanRestartSafe: true,
      };
    }
    ...
    const exactPrimary = (
      ...
      && matchingGeneration
      && exactOptionalQueueReservation
      && (
        (reservation == null && evidence.kind === 'primary-turn')
        || (
          reservation != null
          && evidence.kind === 'dispatch-reservation'
        )
    ...
    if (
      exactPrimary
      && attempt.recoveryState === 'prepared'
      && cancellationProof == null
    ) {
      return {
        action: 'recover',
    ...
    if (exactPrimary && attempt.recoveryState === 'cancelled') {
      if (
        cancellationProof?.kind === 'active-start-cancelled'
        && cancellationProof?.reason === 'clean-restart'
        && Object.keys(cancellationProof).length === 2
      ) {
        return {
          action: 'recover',
    ...
    const exactQueuedCancellation = (
      evidence.kind === 'queued-send'
      && exactAttempt
      && attempt.method === 'queued/send'
      && attempt.recoveryState === 'cancelled'
      && matchingGeneration
      && exactOptionalQueueReservation
      && cancellationProof?.kind === 'queued-send-cancelled'
      && cancellationProof?.reason === 'clean-restart'
      && Object.keys(cancellationProof).length === 2
    );
    if (exactQueuedCancellation) {
      return {
        action: 'recover',
        reason: 'clean-restart-queued-send-cancelled',
        cleanRestartSafe: true,
      };
    }
    return {
