...
/**
 * Resolve the compatibility namespace from canonical runtime/backend identity.
 * Historical stored `tmux` rows are inline. Current config `pm: "tmux"` is
 * normalized to canonical `cli` before it reaches this resolver.
 */
function sessionNamespaceFor({ runtime = 'claude', backend = null } = {}) {
  if (runtime === 'codex') return CODEX_APP_SERVER_NAMESPACE;
  if (runtime !== 'claude') {
    throw new TypeError(`unsupported provider runtime: ${runtime}`);
  }
  return CHANNELS_CLASS.has(backend)
...
function migrateJsonToDb(db, sessionsJsonPath, configChats = {}) {
  ...
  if (dbCount === 0) {
    ...
    try {
      ...
      if (db?.logEvent) {
        try { db.logEvent('sessions-json-malformed', { error_class: err.name || 'Error', quarantined: true }); } catch {}
      }
    ...
    if (!json || typeof json !== 'object' || Array.isArray(json)) {
      ...
      try { fs.renameSync(sessionsJsonPath, quarantine); } catch {}
      if (db?.logEvent) {
        try { db.logEvent('sessions-json-malformed', { error_code: 'not-an-object', quarantined: true }); } catch {}
      }
...
function resolveProviderSessionForSpawn(
  db,
  sessionKey,
  resolved = {},
  { mutateOnDrift = true } = {},
) {
  const namespace = sessionNamespaceFor(resolved);
  if (!db) {
    return {
  ...
  if (resolved.runtime !== 'codex') {
    const legacy = resolveSessionForSpawn(
      db,
      sessionKey,
      resolved,
      { mutateOnDrift },
    );
    const row = legacy.existingSessionId
      ? db.getProviderSession(sessionKey, namespace)
      : null;
    return {
      ...legacy,
      appServerSessionId: null,
      namespace,
      providerSession: row ?? null,
    };
  }

  const row = db.getProviderSession(sessionKey, namespace);
  if (!row) {
    return {
      existingSessionId: null,
      appServerSessionId: null,
  ...
  const fields = CODEX_SPAWN_IDENTITY_FIELDS.filter((field) => (
    after[field] != null && after[field] !== before[field]
  ));
  if (fields.length > 0) {
    if (mutateOnDrift) {
      db.clearProviderSession(sessionKey, namespace);
    }
    return {
      existingSessionId: null,
      appServerSessionId: null,
  ...
  return {
    existingSessionId: row.provider_session_id,
    appServerSessionId: row.app_server_session_id || null,
    namespace,
    drift: null,
    providerSession: row,
  };
...
// Codex model and effort are selected per turn and remain informational on
// the provider-session row. Workspace drift still changes the static runtime
// and security identity, so cwd is the sole invalidating field.
const CODEX_SPAWN_IDENTITY_FIELDS = ['cwd'];
 ...
 * fresh.
 *
 * By default, drift deletes the stale row so the next spawn mints a fresh
 * claude_session_id and `onInit` re-upserts it. Runtime-switch preparation
 * can report the same drift without mutation so a failed replacement leaves
 * the dormant session recoverable.
 *
 * @param {object|null} db          — DB handle (null → fresh spawn)
 * @param {string} sessionKey
 * @param {object} resolved         — freshly-resolved spawn config
 * @param {object} [options]
 * @param {boolean} [options.mutateOnDrift=true]
 *   Delete an incompatible stored row immediately. Runtime-switch
 *   preparation sets this false so a later replacement failure cannot erase
 *   the dormant session.
...
function resolveSessionForSpawn(
  ...
  // Normal cold spawns delete stale rows. Runtime-switch preparation leaves
  // them intact so only a successful target initialization can supersede the
  // active provider-session record.
  if (mutateOnDrift) db.clearSessionId(sessionKey);
  return {
    existingSessionId: null,
    drift: {
      fields: drifted,
