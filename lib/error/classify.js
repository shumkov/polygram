...
function stableCodeShape(kind, userMessage) {
...
function mapCodes(codes, shape) {
  return Object.fromEntries(codes.map((code) => [code, shape]));
}

const CODEX_ERROR_SHAPES = Object.freeze({
  scopeDisabled: stableCodeShape(
    'codexScopeDisabled',
    'Codex is not enabled for this chat',
  ),
  runtimeSelectionIncomplete: stableCodeShape(
    'codexRuntimeSelectionIncomplete',
    'Codex is enabled, but its model, effort, or workspace is not configured for this chat',
  ),
  auth: stableCodeShape(
    'codexAuthUnavailable',
    '🔑 Codex authentication is unavailable. The operator needs to re-authenticate the pinned deployment.',
  ),
  model: stableCodeShape(
    'codexModelUnavailable',
    '⚠️ That Codex model or reasoning effort is unavailable. Choose one from the authenticated Codex catalog.',
  ),
  rateLimit: stableCodeShape(
    'codexRateLimited',
    '⏳ Codex is rate-limited. Codex owns any provider retry; otherwise wait before sending a new request.',
  ),
  protocol: stableCodeShape(
    'codexProtocol',
    '🧩 Codex protocol or runtime pin verification failed. The operator must restore the reviewed deployment.',
  ),
  timeout: stableCodeShape(
    'codexTimeout',
    '⏳ Codex timed out. No automatic retry was started; check Codex status before taking action.',
  ),
  cancelled: stableCodeShape(
    'codexCancelled',
    '🛑 The Codex request was cancelled. No automatic retry was started.',
  ),
  durability: stableCodeShape(
    'codexDurability',
    '⚠️ Codex could not verify this request’s durable outcome. Do not resend automatically; check status and follow operator recovery guidance.',
  ),
  childExit: stableCodeShape(
    'codexChildExit',
    '⚠️ The Codex app-server exited unexpectedly. The outcome may be unknown; wait for operator diagnostics before sending more work.',
  ),
  notSent: stableCodeShape(
    'codexNotSent',
    '↩️ Codex did not receive this request. It is safe to try again.',
  ),
  providerError: stableCodeShape(
    'codexProviderError',
    '⚠️ Codex rejected the request. No automatic retry was started; check Codex status.',
  ),
  busy: stableCodeShape(
    'codexBusy',
    '⏳ Another chat owns this daemon’s native Codex runtime. No session or configuration was changed.',
  ),
});

const CODEX_ERROR_CODES = {
  CODEX_SCOPE_DISABLED: CODEX_ERROR_SHAPES.scopeDisabled,
  CODEX_RUNTIME_SELECTION_INCOMPLETE:
    CODEX_ERROR_SHAPES.runtimeSelectionIncomplete,
  ...mapCodes([
    'CODEX_AUTH_UNAVAILABLE',
  ], CODEX_ERROR_SHAPES.auth),
  ...mapCodes([
    'CODEX_MODEL_UNAVAILABLE',
    'CODEX_EFFORT_UNAVAILABLE',
    'CODEX_MODEL_SELECTION_MISMATCH',
  ], CODEX_ERROR_SHAPES.model),
  ...mapCodes([
    'CODEX_RATE_LIMITED',
    'CODEX_RATE_LIMIT_EXCEEDED',
  ], CODEX_ERROR_SHAPES.rateLimit),
  ...mapCodes([
  ...
  ], CODEX_ERROR_SHAPES.durability),
  ...mapCodes([
    'CODEX_PROCESS_EXITED',
    'CODEX_PROCESS_ERROR',
    'CODEX_TRANSPORT_ERROR',
    'CODEX_PROCESS_CLEANUP_UNVERIFIED',
  ], CODEX_ERROR_SHAPES.childExit),
  CODEX_RPC_NOT_SENT: CODEX_ERROR_SHAPES.notSent,
  CODEX_RPC_ERROR: CODEX_ERROR_SHAPES.providerError,
  CODEX_DAEMON_GENERATION_BUSY: CODEX_ERROR_SHAPES.busy,
};
...
function classify(err) {
  ...
  if (
    ...
    && (
      err?.rpcCode === 429
      || err?.providerCode === 'rate_limit'
      || err?.providerCode === 'rate_limit_exceeded'
    )
  ) {
    return { ...CODEX_ERROR_SHAPES.rateLimit };
  }
  if (code && CODES[code]) {
    return { ...CODES[code] };
  }
  if (typeof code === 'string' && /^CODEX_[A-Z0-9_]+$/.test(code)) {
    return { ...CODEX_ERROR_SHAPES.providerError };
  }
