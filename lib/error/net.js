...
function stripUrlCredentials(url) {
  if (!url) return url;
  // The userinfo span runs to the LAST '@' before the path, not the first.
  // A password containing an unencoded '@' ("alice:p@ss@host") would
  // otherwise be cut at the first one, leaving the rest of the password
  // visible in whatever this was being sanitized for. The class still
  // excludes '/' and whitespace, so the match cannot run past the authority.
