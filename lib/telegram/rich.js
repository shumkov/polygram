...
// Any markdown list is structure: plain stays for a paragraph or two of
// prose, and a reply organized as bullets or numbered items renders as a
// rich document. Line-anchored, marker + space + content — a dash inside a
// sentence or a lone '-' line (divider territory) does not match. Matches
// marked's own list tokenization, so the gate and the renderer agree.
const LIST_ITEM_RE = /^[ \t]{0,3}(?:[-*+]|\d{1,9}[.)])[ \t]+\S/m;
...
// Bounds, both of them the same argument as the media fan-out cap: agent
// output is influenced by content the agent read, so anything unbounded in it
// is a lever. Telegram caps blocks and nesting; it says nothing about nodes
// per block, and a reply can reach five figures without trying.
//
// Past either bound the run flattens — the text is identical, only the
// formatting is lost, which is exactly the degrade the whole feature is
// designed around.
const MAX_STYLE_NODES_PER_BLOCK = 200;
// The API's documented nesting limit is 16 and marked reaches 38 from 25
// emphasis markers, so this sits well below both. Nothing legible needs it:
// bold inside a link inside a list is depth 3.
const MAX_STYLE_DEPTH = 8;

// Only http(s) destinations become link nodes. A typed url node hands
// Telegram a destination to make clickable, which is a trust boundary the
// FLAT rendering never crossed — it discarded the destination entirely. So
// `javascript:`, `data:`, `file:` and anything else keeps the link's text and
// drops the node; the reader loses a hyperlink they could not safely have
// been given.
...
function richTextOf(tokens, depth = 0) {
  const parts = [];
  let nodes = 0;
  ...
  for (const t of tokens || []) {
    ...
    if (t.type === 'text' && Array.isArray(t.tokens)) { absorb(richTextOf(t.tokens, depth)); continue; }
    if (t.type === 'text' || t.type === 'escape') { pushText(t.text ?? ''); continue; }
    if (t.type === 'br') { pushText('\n'); continue; }

    // Past either bound, everything left in this run flattens. Checked here
    // rather than after the walk so a pathological reply is never built at all.
    const exhausted = nodes >= MAX_STYLE_NODES_PER_BLOCK || depth >= MAX_STYLE_DEPTH;

    if (t.type === 'codespan') {
      const text = t.text ?? '';
      if (!text) continue;
      if (exhausted) { pushText(text); continue; }
      nodes += 1;
      parts.push({ type: 'code', text });
      continue;
    }

    const styled = INLINE_STYLE_NODES.get(t.type);
    if (styled) {
      if (exhausted) { pushText(plainTextOf([t])); continue; }
      const inner = richTextOf(t.tokens, depth + 1);
    ...
    if (t.type === 'link') {
      // The flat label decides the guards below and is the fallback for every
      // branch that declines to emit a node — a rejected link still shows its
      // text.
      const flatLabel = plainTextOf(t.tokens) || t.text || '';
      if (exhausted
          || typeof t.href !== 'string'
          || !SAFE_URL_RE.test(t.href)
          || !flatLabel) {
        // A link with no destination, a non-http scheme, or no text is not a
        // link node. Keep what the reader can see, drop what they cannot
        // safely be handed.
        pushText(flatLabel || plainTextOf([t]));
        continue;
      }
      // A styled label — `[**bold** link](…)` — is ordinary markdown, and a
      // probe confirmed nested nodes survive inside a url node. Counted at
      // depth+1 so a label cannot dodge the nesting ceiling.
      const label = richTextOf(t.tokens, depth + 1);
      nodes += 1;
      parts.push({
        type: 'url',
        text: isEmptyRichText(label) ? flatLabel : label,
        url: t.href,
      });
      continue;
    }

    // Everything else — images, raw html, anything marked grows later —
    // keeps the flattened form. Images especially: they are block-level media
    // on this pipeline and must never become inline nodes.
...
// The visible text of a rich-text value, whatever shape it is in: the string
// itself, or every string a node tree would put on screen.
function flattenRichText(value) {
  if (typeof value === 'string') return value;
  if (Array.isArray(value)) return value.map(flattenRichText).join('');
  if (value && typeof value === 'object') return flattenRichText(value.text ?? '');
  return '';
}

/**
 * Strip styling from an ALREADY-RENDERED block tree, leaving everything else
 * — structure, media, captions, alignment — bit for bit as it was.
 *
 * Used as the retry rung after a refused styled payload. Flattening in place
 * rather than re-rendering the source matters for two reasons: nothing is
 * resolved a second time (media stays exactly the values that were checked,
 * with no second pass over the filesystem), and the retry differs from the
 * refused payload in EXACTLY ONE respect. That is what makes a
 * refused-then-accepted pair evidence about styling rather than about
 * whatever else a re-render might have changed.
 */
function flattenStyledBlocks(blocks) {
  return (blocks || []).map((b) => {
    if (!b || typeof b !== 'object') return b;
    const out = { ...b };
    if (Array.isArray(b.text)) out.text = flattenRichText(b.text);
    if (b.caption && Array.isArray(b.caption.text)) {
      out.caption = { ...b.caption, text: flattenRichText(b.caption.text) };
    }
    if (Array.isArray(b.cells)) {
      out.cells = b.cells.map((row) => (row || []).map((cell) => (
        cell && Array.isArray(cell.text) ? { ...cell, text: flattenRichText(cell.text) } : cell
      )));
    }
    if (Array.isArray(b.blocks)) out.blocks = flattenStyledBlocks(b.blocks);
    if (Array.isArray(b.items)) {
      out.items = b.items.map((item) => {
        if (!item || typeof item !== 'object') return item;
        const next = { ...item };
        if (Array.isArray(item.text)) next.text = flattenRichText(item.text);
        if (Array.isArray(item.blocks)) next.blocks = flattenStyledBlocks(item.blocks);
        return next;
      });
    }
    return out;
...
/** Does this tree carry any typed node at all? */
function blocksAreStyled(blocks) {
  for (const b of blocks || []) {
    if (!b || typeof b !== 'object') continue;
    if (Array.isArray(b.text) || Array.isArray(b.caption?.text)) return true;
    if (Array.isArray(b.cells) && b.cells.some((row) => (row || []).some((c) => Array.isArray(c?.text)))) return true;
    if (Array.isArray(b.blocks) && blocksAreStyled(b.blocks)) return true;
    if (Array.isArray(b.items)) {
      for (const item of b.items) {
        if (Array.isArray(item?.text)) return true;
        if (Array.isArray(item?.blocks) && blocksAreStyled(item.blocks)) return true;
      }
    }
  }
  return false;
...
function rewriteImageConstructs(line, onImage, onUnparsed = () => '') {
  ...
  while (i < line.length) {
    ...
    // Unparseable: everything from here is the tail of a destination we could
    // not read, so the default drops it. A caller whose text is COMPLETE can
    // decide otherwise — see stripMediaMarkdown's keepPathlessFragments.
    if (!parsed) return out + onUnparsed(line.slice(next));
    out += onImage(parsed);
    i = parsed.end;
  }
  return out;
  ...
  switch (token.type) {
    ...
    case 'html': {
      ...
      if (m) {
        ...
        const summary = flattenSummaryMarkdown((m[1] || '').replace(HTML_TAG_STRIP_RE, '').trim());
...
// A <summary> is authored as markdown like everything else — "**Логи** с
// `кодом`" must flatten to its text, not ship its markers into the chip.
// The summary field is a plain string in the block schema, so styling
// does not apply here; flattening through the lexer is the summary
// counterpart of plainTextOf on inline runs.
function flattenSummaryMarkdown(s) {
  if (!s || !/[*_`~[\]]/.test(s)) return s;
  try {
    const tokens = _lexer.lexer(s);
    const flat = tokens.map((t) => plainTextOf(t.tokens || [t])).join(' ').trim();
    return flat || s;
  } catch {
    return s;
  }
...
function listItemFromToken(item, listToken, index, ctx = FLAT) {
  ...
  // A LOOSE task item (blank line between items) carries its content as
  // nested paragraph BLOCK tokens with an empty inline leader, so the
  // leader-branch strip above never sees the marker. Strip it from the
  // item's first paragraph wherever it came from — idempotent for tight
  // items whose leader was already stripped.
  if (item.task === true && blocks[0]?.type === 'paragraph') {
    blocks[0] = { ...blocks[0], text: stripTaskMarker(blocks[0].text, true) };
  }
...
function blocksFromTokens(tokens, ctx = FLAT) {
  ...
  while (i < list.length) {
    ...
    if (t.type === 'html' && DETAILS_OPEN_RE.test(raw) && !DETAILS_CLOSE_RE.test(raw)) {
      ...
      const summary = summaryMatch ? flattenSummaryMarkdown(summaryMatch[1].replace(HTML_TAG_STRIP_RE, '').trim()) : '';
      const inner = [];
      let closeRaw = null;
      let j = i + 1;
      for (; j < list.length; j++) {
...
// The same fragment, narrowed to the ones that actually expose a path: the
// destination must have opened AND contain a separator. For text that is
// COMPLETE when it arrives — a tool edit, not a stream tick — nothing can
// finish a dangling "![alt", so it is literal content the author wrote and
// cutting it silently eats the tail of their sentence. "![a](/User…" still
// dies, which is the only shape that leaks a path.
const TRAILING_IMAGE_PATH_FRAGMENT_RE = /!\[[^\]\n]*\][ \t]*\([^)\n]*\/[^)\n]*$/;

/**
 * @param {string} markdown
 * @param {object} [opts]
 * @param {boolean} [opts.keepPathlessFragments=false] — cut only unterminated
 *   fragments that carry a path separator. For a complete text; leave it off
 *   for anything still streaming, where "![alt" IS a path fragment that has
 *   not arrived yet.
 */
function stripMediaMarkdown(markdown, { keepPathlessFragments = false } = {}) {
  ...
  for (const line of lines) {
    ...
    let clean = rewriteImageConstructs(
      ...
      // Keep an unreadable tail only when there is no path anywhere in it. A
      // tail with a separator may be a destination this parser could not read
      // but a chat client would still show — dropping it stays the rule. One
      // test decides the whole tail, so `![a] ![b](/real/path.png)` (parse
      // fails at the first construct) cannot slip a path through as literal
      // text.
      keepPathlessFragments ? (tail) => (/[/\\]/.test(tail) ? '' : tail) : undefined,
    ...
    clean = clean.replace(
      keepPathlessFragments ? TRAILING_IMAGE_PATH_FRAGMENT_RE : TRAILING_IMAGE_FRAGMENT_RE,
      '',
    );
    out.push(clean);
  }
...
/**
 * partialVisibleExtent(markdown) -> number of leading SOURCE characters that a
 * partial render actually puts on screen.
 *
 * Partial mode holds the last top-level block back (it may be half-written),
 * so the reader sees the source behind blocks 0..n-2 and nothing of the rest.
 * Anything that needs to reason about "what has the reader already seen"
 * has to ask the renderer, because only the renderer knows where its own
 * blocks begin and end — a blank-line heuristic disagrees with it in both
 * directions (a fence or a loose list carries internal blank lines; adjacent
 * headings carry none).
 *
 * Returns the extent of the largest token prefix that renders no MORE than
 * the displayed blocks, so the answer is exact wherever a token maps to one
 * block and errs toward reporting less rather than claiming the reader saw
 * something they did not.
 */
function partialVisibleExtent(markdown) {
  if (typeof markdown !== 'string' || markdown.length === 0) return 0;
  const tokens = _lexer.lexer(markdown);
  const displayed = blocksFromTokens(tokens, FLAT).length - 1;
  // One block (or none) is emitted whole rather than held back, so all of it
  // reached the screen — same rule toTelegramRichBlocks applies.
  if (displayed < 1) return markdown.length;

  let extent = 0;
  let consumed = 0;
  for (let n = 1; n <= tokens.length; n += 1) {
    consumed += (tokens[n - 1].raw || '').length;
    if (blocksFromTokens(tokens.slice(0, n), FLAT).length > displayed) break;
    extent = consumed;
  }
  return extent;
}
...
// A content error that names a LIMIT rather than a shape. Worth separating
// because the styling retry flattens the payload, which makes it smaller and
// shallower as well as unstyled — so an oversized reply that succeeds on the
// retry would otherwise read as "this server refuses typed nodes", and two of
// them would disable styling for the process. Retry either way; count only
// the one where styling is the plausible cause.
const RICH_LIMIT_ERR_RE = /too (?:many|long|large|big)|_TOO_(?:MANY|LONG|LARGE)|exceed|limit/i;

function isRichLimitError(err) {
  if (!err) return false;
  return RICH_LIMIT_ERR_RE.test(errorMessage(err));
}
