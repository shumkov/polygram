...
// ─── Media (photo / collage / slideshow) ─────────────────────────────
//
// Image markdown maps to Telegram photo blocks; <tg-collage>/<tg-slideshow>
// wrapper tags (Telegram's own rich-markdown dialect) group images into
// collage/slideshow blocks. The renderer emits internal DESCRIPTOR
...
function mediaPlaceholderText(d, { unavailable = false } = {}) {
  if (unavailable) return d.caption ? `${d.caption} (image unavailable)` : '(image unavailable)';
  return d.caption ? `🖼 ${d.caption}` : '🖼 image';
}
...
function photoBlockFrom(result, caption) {
  const block = { type: 'photo', photo: { type: 'photo', media: result.media } };
  if (caption) block.caption = { text: caption };
  return block;
}

// Replace descriptor blocks with real photo/collage/slideshow blocks
...
function substituteMediaBlocks(blocks, results, cursor = { i: 0 }) {
  ...
  for (const b of blocks || []) {
    if (b.type === '_media') {
      ...
      cursor.i += 1;
      if (r && r.media) out.push(photoBlockFrom(r, b.caption));
      ...
      if (!results) {
        ...
        out.push({ type: 'paragraph', text: `🖼 ${b.kind} (${b.items.length} images)` });
        continue;
      }
      const photos = [];
      let unavailable = 0;
      for (const it of b.items) {
        ...
        cursor.i += 1;
        if (r && r.media) photos.push(photoBlockFrom(r, it.caption));
        else unavailable += 1;
      }
      if (photos.length === 1) out.push(photos[0]);
      else if (photos.length > 1) out.push({ type: b.kind, blocks: photos });
      if (unavailable > 0) {
        out.push({ type: 'paragraph', text: `(${unavailable} image${unavailable === 1 ? '' : 's'} unavailable)` });
      }
...
// Count media attachments in an emitted block tree (photo blocks plus
// collage/slideshow children) — the media_count instrumentation field.
 ...
 * index-aligned with the descriptors; each result is `{ media }` (an
 * https URL string or a JSON-safe `{ source: '<realpath>' }` envelope
 * that rich-edit.js later materializes into a grammy InputFile) or a
 * falsy/rejected marker. NEVER called in partial mode — streaming ticks
 * must not touch the filesystem; images render as placeholder
