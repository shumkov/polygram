...
// marked token type → node discriminator. Deliberately small: everything
// absent from this map keeps flattening to its text, which is always safe.
...
// a nested walk can narrow it (list items do — see below) and so nothing here
// depends on render order.
//
// PARAGRAPHS and HEADINGS only. A live probe found list-item and table-cell
// styling PRESERVED on one run and REJECTED on an identical repeat, which is
// rate pressure's signature rather than a field's answer; until a paced run
// says otherwise those two flatten. Widening this is a one-line change per
// field once that evidence exists.
...
function listItemFromToken(item, listToken, index) {
  ...
  // A list item's text IS a nested paragraph, so it would inherit styling
  // from the paragraph branch. Narrowed here rather than there: the field
  // whose evidence is contradictory is the LIST, and the paragraph branch
  // should not have to know that.
  const nestedBlocks = blocksFromTokens(blockTokens, FLAT);
  const blocks = [];
  if (inlineHasMedia(inline)) {
    blocks.push(...blocksFromInlineTokens(inline, FLAT));
    if (!blocks.length && !nestedBlocks.length) blocks.push({ type: 'paragraph', text: '' });
  } else {
    let leaderText = plainTextOf(inline);
    if (item.task) leaderText = leaderText.replace(TASK_MARKER_RE, '');
