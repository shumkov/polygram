...
function richTextOf(tokens, depth = 0) {
  ...
  for (const t of tokens || []) {
    ...
    if (t.type === 'link') {
      // The label stays a STRING until a probe says nested nodes survive
      // inside a url node. It was never asked: the probe sent url nodes with
      // string text only, so emitting `{type:'url', text:[…]}` would be
      // shipping an unverified shape on the strength of a different one.
      const label = plainTextOf(t.tokens) || t.text || '';
      if (exhausted
