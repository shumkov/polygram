...
const { getSessionKey } = require('../session-key');
...
//
// sendRichMessage is untracked for a different reason: it is a *try*. Any
// failure degrades to the plain chunked path, which inserts its own rows —
// so a tracked row here would persist the failed attempt alongside the
// successful fallback and the agent's preloaded history would show the
// same answer twice. The rich sender inserts one row itself, on success only.
...
async function send({ bot, method, params, db = null, meta = {}, logger = console, config = null }) {
  ...
  // Validate any rich payload by its PRESENCE, not by the method carrying
  // it. Keyed on the verb, this guard would wave through the same payload
  // under any other method name — which is what a new caller looks like.
  if (params.rich_message != null) {
    ...
    // Typed blocks are the only rich payload polygram emits. The `html` and
    // `markdown` alternatives would hand Telegram a string it re-parses,
    // reintroducing the markup-injection surface the block renderer exists
    // to close — and nothing downstream sanitizes them.
    if (rich.html != null || rich.markdown != null) {
      throw new Error(
        `telegram ${method}: rich_message.html/markdown is not supported — send typed blocks`,
      );
    }
  } else if (method === 'sendRichMessage') {
    throw new Error(`telegram ${method}: rich_message is missing`);
  }

  // A rich send/edit carries `rich_message` instead of `text`, so the
  // plain-text guard applies only when there is no rich payload.
  ...
  //
  // Rich bubbles have never honored it, from two directions: sendRichMessage
  // is not one of the methods above, and the rich form of editMessageText
  // carries a meta built for the rich renderer that never had the flag in it.
  // So a chat with the opt-out still gets preview cards under any rich reply.
  // A bubble that OPENS rich only starts that gap one frame earlier than one
  // that upgraded into it. Recorded, not fixed — the two paths would need the
  // option threaded through the rich renderers, and no rich verb is known to
  // accept it.
  ...
  try {
    ...
    if (rowId != null && db) {
      ...
      try {
        db.logEvent('telegram-api-error', {
          chat_id: chatId,
          method,
          error_code: err.code || err.name || 'error',
          error_len: typeof safe === 'string' ? safe.length : 0,
        });
      }
...
const OPERATIONAL_UI_METHODS = new Set([
  'sendChatAction',
  'setMessageReaction',
]);
const REPLY_BEARING_METHODS = new Set([
  'deleteMessage',
  'editMessageCaption',
  'editMessageMedia',
  'editMessageReplyMarkup',
...
function defaultDeliveryClass(method) {
  if (OPERATIONAL_UI_METHODS.has(method)) return 'operational-ui';
  if (REPLY_BEARING_METHODS.has(method)) return 'reply-bearing';
  return null;
}

function deliverySessionKey(params, meta, config) {
  if (meta.sessionKey != null) return meta.sessionKey;
  if (params.chat_id == null) return null;
  const chatId = String(params.chat_id);
  const threadId = params.message_thread_id == null
    ? null
    : String(params.message_thread_id);
  return getSessionKey(chatId, threadId, config?.chats?.[chatId]);
}

function deliverySourceMsgId(params, meta) {
  const value = meta.sourceMsgId ?? params.reply_parameters?.message_id ?? null;
  if (value == null) return null;
  const numeric = Number(value);
  return Number.isSafeInteger(numeric) ? numeric : null;
}

function createSender(db, logger = console, config = null, deliveryBarrier = null) {
  return (bot, method, params, meta = {}) => {
    ...
    if (!deliveryBarrier) return operation();
    return deliveryBarrier.run({
      sessionKey: deliverySessionKey(params, meta, config),
      sourceMsgId: deliverySourceMsgId(params, meta),
      deliveryClass: meta.deliveryClass ?? defaultDeliveryClass(method),
    }, operation);
  };
}
