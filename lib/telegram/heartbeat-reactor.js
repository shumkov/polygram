 ...
 *   Used ONLY with CliProcess. SdkProcess and TmuxProcess keep
 ...
 *   on 'bridge-disconnected'  → stop permanently, clear reaction (channels-only event;
 *                               closes the small visible gap between bridge-disconnect
 *                               and the kill-then-close that pm-manager triggers)
...
class HeartbeatReactor {
  /**
   * @param {object} opts
   * @param {EventEmitter} opts.process            — CliProcess (or any Process subclass)
  ...
  _bind() {
    ...
    // bridge-disconnected fires on the channels driver before pm-manager
    // kills the process and 'close' lands. Stop here too so the cycling
    // emoji clears immediately rather than at the kill-then-close moment.
    this._onBridgeDisconnect = () => this.stop();
    ...
    this.process.on('bridge-disconnected', this._onBridgeDisconnect);
  ...
  _unbind() {
    ...
    this.process.off('bridge-disconnected', this._onBridgeDisconnect);
    for (const ev of LIVENESS_EVENTS) {
