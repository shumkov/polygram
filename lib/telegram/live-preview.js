 ...
 * @param {(text: string, ctx: {chatId, threadId}) => string} [deps.projectConsumedText]
 *   — applied to the text a CONSUMED reply finalizes the preview with. The
 *   bubble is rendered by the streamer, whose media resolver answers to the
 *   interactive path's rules rather than the reply tool's, so media must not
 *   ride in on this branch. Identity by default; a decline is unaffected, so
 *   whatever the next strategy receives is still the text the agent wrote.
 */
function createDeliverTextFactory({
  ...
  logger = console,
  botName = null,
  projectConsumedText = null,
} = {}) {
  return function makeDeliverText({ sessionKey, chatId, threadId, interim, turnId } = {}) {
    ...
    async function consume({ text: authored }) {
      // What this branch can deliver, which is not the same as what was
      // written. Consuming means the STREAMER renders the bubble, under the
      // interactive path's media rules — wider roots and a wider fan-out than
      // the reply tool's, reached without any of its gates. Projecting media
      // out here is what keeps "a reply-tool reply uploads only what the reply
      // tool would" true on both branches. Media-free text is untouched, so
      // every other reply is byte-identical.
      //
      // Used for the coverage bookkeeping too: what was delivered is what the
      // orphan check must compare against, not what was authored.
      //
      // Follow-up, deliberately not done here: thread the dispatcher's own
      // media wiring (its per-call roots and ceilings) into finalize, so a
      // consumed reply can render inline media under the same boundary the
      // reply tool applies instead of degrading to captions.
      const text = typeof projectConsumedText === 'function'
        ? projectConsumedText(authored, { chatId, threadId })
        : authored;
