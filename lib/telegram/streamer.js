 ...
 */
// Everything the reader has seen settle, in a PLAIN bubble: the complete
// lines of what was rendered. The plain path shows the whole text, so there
// is no held-back block to exempt — but the line being typed is not settled,
// and a model rewriting the sentence it is mid-way through is ordinary
// partial-mode behaviour rather than losing anything.
function completeLines(shown) {
  const i = shown.lastIndexOf('\n');
  return i === -1 ? '' : shown.slice(0, i + 1);
}

// Lazy so the streamer keeps loading for callers (and tests) that never touch
// the rich path, and so requiring rich.js stays out of the module's own load.
let _partialVisibleExtent = null;
function defaultRichVisibleExtent(markdown) {
  if (!_partialVisibleExtent) {
    ({ partialVisibleExtent: _partialVisibleExtent } = require('./rich'));
  }
  return _partialVisibleExtent(markdown);
}
 ...
 *
 * CRLF is folded first, and not for tidiness: a boundary located by newline
 * never matches `\r\n`, so a CRLF document would find none at all and
 * silently switch the guard off — the one failure mode a guard must not have.
 * Trailing whitespace goes too: a snapshot that loses a blank line off its end
 * has lost nothing anyone was reading.
 */
...
function createStreamer({
  ...
  // (markdown) -> number of leading source characters a PARTIAL rich render
  // displays. Injected for the same reason toRichPayload is: the streamer
  // does not own the renderer. Defaults to lib/telegram/rich's answer, which
  // is the renderer every polygram caller actually uses; a caller with no
  // rich path never reaches it.
  richVisibleExtent = defaultRichVisibleExtent,
} = {}) {
  ...
  // The source behind what the reader can currently see, recorded by whichever
  // render put it there. The append-only guard pins exactly this: a rich frame
  // contributes the source of the blocks it displayed (the held-back tail was
  // never shown), a plain frame the complete lines it displayed. Empty means
  // nothing has been rendered yet, and nothing may be pinned.
  let pinnedSource = '';
  ...
  async function onChunk(text) {
    ...
    //
    // The pin is what the RENDERER put on screen, recorded when it did — not
    // a boundary re-derived from the accepted text afterwards. Only the
    // renderer knows where its own blocks begin, and a snapshot can be
    // accepted while its edit is still throttled, so "last accepted" and
    // "on screen" are not the same text. Empty until something has actually
    // been rendered, which is what keeps a turn that has not opened a bubble
    // from pinning a greeting nobody saw.
    const pinned = canonicalizeForGuard(pinnedSource);
    if (pinned && !canonicalizeForGuard(text).startsWith(pinned)) {
      try {
        onNonCumulativeSnapshot({
      ...
      try {
        ...
        if (msgId == null) {
          ...
          if (res?.wentRich === false) {
            ...
            currentRichJson = null;
            notePinned(shownPlainSource(plainText, text), false);
          } else {
            currentRichJson = JSON.stringify(openRich.blocks);
            notePinned(text, true);
          }
        } else {
          notePinned(shownPlainSource(plainText, text), false);
        }
  ...
  // Record what a frame just put on screen, so the guard pins that and only
  // that. `shownSource` is the source the frame rendered; `wasRich` picks
  // which part of it the reader actually saw.
  // The source prefix a plain frame actually displayed. An untruncated frame
  // showed all of it; a truncated one showed everything before the "..." the
  // truncation appended, and pinning past that would pin characters the
  // reader could not see.
  function shownPlainSource(rendered, source) {
    return rendered === source ? source : rendered.slice(0, -3);
  }

  function notePinned(shownSource, wasRich) {
    // partialVisibleExtent answers from the renderer's own token/block stream,
    // so the pin cannot disagree with what was drawn. Absent (a caller with no
    // rich path at all), a rich frame is impossible anyway.
    const visible = wasRich
      ? shownSource.slice(0, richVisibleExtent ? richVisibleExtent(shownSource) : 0)
      : shownSource;
    // Settled, not merely visible. A rich frame with nothing held back — one
    // block, which the renderer draws rather than showing an empty bubble —
    // puts the block being WRITTEN on screen, and the reader is watching it
    // resize. Its finished lines have settled; the line in progress has not,
    // exactly as on the plain path.
    pinnedSource = completeLines(visible);
  }
  ...
  async function flushOnce() {
    ...
    if (richPayload) {
      ...
      // redundant strip edit; wrong the other way is permanent — which is
      // also why this only ever SETS: a removal is believed once Telegram
      // confirms it, never before.
      if (marker) currentMarkerShown = true;
      try {
        ...
        currentMarkerShown = !!marker;   // confirmed: this frame is on screen
        if (wentRich) {
          currentRichJson = json;
          notePinned(sourceText, true);
    ...
    lastEditTs = clock();
    if (marker) currentMarkerShown = true;   // before the await — see the rich branch
    try {
      ...
      notePinned(shownPlainSource(next, sourceText), false);
    } catch (err) {
  ...
  async function sealBubble(snapshot) {
    ...
    if (payload?.usedRich) {
      ...
      await editWithRetry(targetMsgId, {
        rich: true,
    ...
    // A RICH bubble whose final render declined rich still has to lose its
    // marker, and it must not lose its document doing so: toRichPayload can
    // render rich while partial and decline at partial:false (rich.js demotes
    // a media-only document whose media never resolved, gated on !partial so
    // it cannot fire during the live ticks). The blocks already on screen are
    // recorded — re-send exactly those, which differ from what is displayed
    // only by the marker that was appended after them.
    if (snapshot.currentRichJson !== null) {
      await editWithRetry(targetMsgId, {
        rich: true,
        blocks: JSON.parse(snapshot.currentRichJson),
        sourceText: snapshot.sourceText,
        phase: 'seal',
        hadReplyAnchor: targetHadReplyAnchor,
      });
    ...
    await editWithRetry(targetMsgId, truncateForLive(snapshot.sourceText));
  }

  // A seal is the LAST edit a detached bubble will ever get — appendSealJob
  // logs a failure and moves on, and no turn-end pass revisits it. A transient
  // failure would therefore strand whatever the seal was removing, forever, so
  // it is worth exactly one more attempt.
  // Silent on the first failure by design: appendSealJob reports the one that
  // sticks, and it reports the error CLASS rather than the message because a
  // seal error can name a media path. A retry line carrying err.message would
  // put that path in the log this module works to keep it out of.
  async function editWithRetry(targetMsgId, payload) {
    try {
      await edit(targetMsgId, payload);
    } catch {
      await edit(targetMsgId, payload);
    }
  }
  ...
  function forceNewMessage() {
    ...
    currentMarkerShown = false;
    pinnedSource = '';   // a new bubble has shown the reader nothing yet
    latestText = '';
    state = 'idle';
    lastEditTs = 0;
    flushQueued = false;
