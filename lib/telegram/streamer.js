...
function createStreamer({
  ...
  onRichUpgrade = () => {},
  // Fires when a snapshot is refused for losing text the reader had already
  // seen. Receives { prevLen, newLen, violations }; the caller owns the chat
  // and turn identifiers, which this module has no business knowing.
  ...
  let nonCumulativeCount = 0; // snapshots refused for losing seen text
  ...
  async function onChunk(text) {
    ...
    const committed = committedPrefix(latestText);
    if (committed && !text.startsWith(committed)) {
      nonCumulativeCount += 1;
      try {
        onNonCumulativeSnapshot({
          prevLen: latestText.length,
          newLen: text.length,
          violations: nonCumulativeCount,
        });
      } catch (err) {
  ...
  function resolveComposingMarker(contentLen, cap) {
    if (!toComposingMarker) return null;
    let marker;
    try {
      marker = toComposingMarker();
    } catch (err) {
    ...
    // One budget for both shapes. The rich block's rendered weight is the same
    // line of text the plain suffix carries, and a single measurement is what
    // keeps the two paths from disagreeing about whether it fits.
    return contentLen + marker.plainSuffix.length <= cap ? marker : null;
  }
  ...
  async function flushOnce() {
    ...
    if (richPayload) {
      ...
      const marker = resolveComposingMarker(sourceText.length, richMaxLen);
    ...
    const marker = resolveComposingMarker(next.length, maxLen);
  ...
  async function sealBubble(snapshot) {
    ...
    // No rich render to seal with (plain bubble, over the rich cap, or content
    // that stopped qualifying): the content alone is the same text the bubble
    // already shows, minus the marker. truncateForLive keeps the plain path's
    // existing cap contract rather than inventing a second one here.
