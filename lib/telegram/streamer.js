...
function createStreamer({
  ...
  // already renders as structure — the same rich shape `edit` accepts:
  // { rich: true, blocks, sourceText, plainText, phase: 'open' }. A caller
  // that cannot open rich delivers `plainText` instead and reports
  // `wentRich: false`; the streamer then holds PLAIN state so the next
  // flush edits the bubble it actually has. An absent `wentRich` means the
  // payload landed as sent (backward compatible with plain-only callers).
  send,
  ...
  async function sealBubble(snapshot) {
    ...
    if (targetMsgId == null || snapshot.sourceText.length > richMaxLen || !toRichPayload) return;

    const payload = toRichPayload(snapshot.sourceText, { partial: false });
    if (!payload?.usedRich || !Array.isArray(payload.rescueEntries)
      || payload.rescueEntries.length === 0) {
      return;
    }
    const json = JSON.stringify(payload.blocks);
    if (json === snapshot.currentRichJson) return;
