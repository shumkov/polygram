...
function createRichSender({
  ...
  return async function sendRich({
    ...
    if (messageId == null) {
      ...
      return { wentRich: false, fallback: 'content-error' };
    }
    ...
    try {
      // The send path is ~80% of output, so without this the shared file_id
      // cache would only ever be populated by the streamer and every reply
      // would re-upload the same unchanged file.
