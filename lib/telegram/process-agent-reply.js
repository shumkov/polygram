 ...
 * @param {Function} [opts.deliverText]      — optional delivery strategy replacing ONLY the
 *   chunk+deliver step. Called as ({ text, chatId, threadId, replyToMessageId, meta }) and
 *   returns { handled, text?, sent?, failed?, results? }. `handled: true` makes its result
 *   the deliverResult; `handled: false` falls through to chunk+deliver, using `text` as the
 *   body when the strategy supplied a replacement. Runs after parse/redact/sanitize and
 *   before stickers/reactions, exactly once per reply. Callers that omit it deliver exactly
 *   as before.
 * @param {(text: string) => string} [opts.sanitizeFallbackText] — projection applied
 *   to the body the chunked path delivers, whatever route led there. Deliberately
 *   NOT owned by the delivery strategy: a chat's text hygiene must survive that
 *   strategy failing to build, failing to run, or simply declining.
 * @param {(s: string) => string} [opts.redactError] — strips bot tokens from a logged
 *   strategy error; network error shapes can embed the request URL
 ...
 * @returns {Promise<{ deliverResult: ?object, stickersSent: number, reactionsApplied: number,
 *   reactionsDropped: number, auxiliaryFailures: number, deliveryAmbiguous: boolean,
 *   sanitizerReplaced: boolean, secretsRedacted: number,
 *   emptiedByStrategy?: boolean }>} — `emptiedByStrategy` marks a reply whose text
 *   projected away entirely, so nothing was delivered and the caller must decide
...
async function processAndDeliverAgentText({
  ...
  // 3. Deliver text. An injected strategy gets first refusal — it sits HERE,
  //    inside the pipeline, so redaction/sanitize run exactly once and the
  //    sticker/reaction steps below still fire whichever way delivery went.
  //    Declining (or failing) falls through to the chunked path, which is
  //    the only delivery guarantee either branch relies on.
  if (parsed.text) {
    let body = parsed.text;
    if (typeof deliverText === 'function') {
      let strategy = null;
      try {
        strategy = await deliverText({
          text: body,
          chatId,
          threadId,
          replyToMessageId,
        ...
        // A broken strategy must cost structure, never the message.
        logger?.error?.(`[${source}] delivery strategy failed: ${err.message}`);
        logEvent?.('deliver-strategy-error', {
          chat_id: chatId, thread_id: threadId, source, session_key: sessionKey,
      ...
      } else if (typeof strategy?.text === 'string') {
        body = strategy.text;
      }
    }
    // Applied on every route to the chunked path — strategy declined, threw,
    // was never built, or rewrote the body already (the projection is
    // idempotent). This is the only place that guarantees it.
    if (!summary.deliverResult && typeof sanitizeFallbackText === 'function') {
      try {
        body = sanitizeFallbackText(body) ?? '';
      } catch (err) {
        logger?.error?.(`[${source}] fallback projection failed: ${redactError(err.message)}`);
      }
    }

    // Nothing deliverable survived, and sending the empty string would trip
    // the API's empty-text guard. Record it loudly: a caller that reports
    // success here leaves the user with silence and the agent believing it
    // answered.
    if (!summary.deliverResult && !body) {
      summary.emptiedByStrategy = true;
      logEvent?.('deliver-empty-after-strategy', {
        chat_id: chatId, thread_id: threadId, source, session_key: sessionKey,
      });
    }

    if (!summary.deliverResult && body) {
      const chunks = chunkMarkdownText(body, chunkBudget);
  ...
  if (parsed.sticker) {
    try {
      ...
      summary.auxiliaryFailures += 1;
      summary.deliveryAmbiguous = true;
  ...
  try {
    await tg(bot, 'setMessageReaction', {
      ...
      ...meta,
      sessionKey,
      sourceMsgId: meta.sourceMsgId ?? replyToMessageId,
