 ...
 *
 * Asked for bytes (`withBytes`), the check binds to a DESCRIPTOR rather than
 * a path: open, fstat that descriptor, compare, and read through it. What is
 * uploaded is then provably the bytes that were fingerprinted — a path-backed
 * handle is re-opened by name at request-build time, and again on every
 * retry, so a file swapped after the check but before (or between) those
 * reads would upload its new contents under the old verdict. The buffer also
 * makes a 429 retry re-send the same bytes instead of re-reading the path.
 * Cost, stated: the reply's media is held in memory for the duration of the
 * send, bounded by the per-reply budget (maxTotalMediaBytes).
 *
...
function createMediaPreflight({
  ...
  function preflight(media, kind = 'photo', { preferFileId = true, withBytes = false } = {}) {
    ...
    // A cached id needs no bytes: Telegram already holds the file.
    if (preferFileId && fileId) return { ok: true, value: fileId };
    if (!withBytes) return { ok: true, value: { source: media.source } };

    let fd;
    try { fd = fs.openSync(check.resolved, 'r'); }
    catch {
      evict(kind, media);
      return { ok: false };
    }
    try {
      // The path check above proved the NAME resolves somewhere allowed; this
      // proves the OPEN FILE is still the one that was fingerprinted. Between
      // the two, the name could have been repointed.
      if (statFingerprint(fs.fstatSync(fd, { bigint: true })) !== media.fingerprint) {
        evict(kind, media);
        return { ok: false };
      }
      return { ok: true, value: { source: media.source, bytes: fs.readFileSync(fd) } };
    } catch {
      evict(kind, media);
      return { ok: false };
    } finally {
      try { fs.closeSync(fd); } catch { /* the read already decided the outcome */ }
    }
  }
  ...
  async function deletePlaceholder(messageId, meta = {}) {
    try {
      await tg(bot, 'deleteMessage', {
        ...
        sourceMsgId,
        ...meta,
        source: 'bot-reply-rich-media-placeholder-delete',
        botName,
      });
      return true;
    } catch (err) {
  ...
  async function flushPartialDeliveryWarning(meta = {}) {
    ...
    try {
      await tg(bot, 'sendMessage', {
        ...
        sourceMsgId,
        ...meta,
        source: 'partial-delivery-warning',
        botName,
      });
    } catch (err) {
