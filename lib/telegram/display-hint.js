...
// Media guidance is separated out because it is NOT true on every path. The
// streamer renders inline media today; the reply tool renders rich text on
// media-stripped input, so there an image reaches the user as its caption and
// nothing else. Teaching syntax the delivering path discards makes the agent
// author constructs that silently vanish, so each caller states what its own
// path can do.
