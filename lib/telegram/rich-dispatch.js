...
function createRichDeliveryFactory({
  ...
  return function makeDeliverText({ chatId, threadId, allowedRoots = null } = {}) {
    return async function deliverText({ text, replyToMessageId = null, meta = {} } = {}) {
      ...
      // the styling, so re-render without it before giving up the structure.
      // Losing every heading and table because one code span was unwelcome
      // would be the worse trade, and the retry is also the experiment: if
      // the flat version lands, styling is what was refused.
      if (styled && out && !out.wentRich && out.fallback === 'content-error') {
        const flattened = render(media?.resolveMedia ?? null, false);
        if (flattened) {
          styled = false;
          const retried = await attempt(flattened);
          if (retried?.wentRich) {
