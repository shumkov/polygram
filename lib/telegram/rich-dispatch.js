...
// A block text field is a string OR an array of strings and typed nodes, and
// the nodes nest. Reduce either to the strings a reader would SEE, so a
// predicate about visible text cannot be dodged by adding styling to the
// paragraph that carries the leak.
function visibleStrings(value, out = []) {
  if (typeof value === 'string') { out.push(value); return out; }
  if (Array.isArray(value)) {
    for (const part of value) visibleStrings(part, out);
    return out;
  }
  if (value && typeof value === 'object') {
    if (typeof value.text === 'string' || Array.isArray(value.text)) visibleStrings(value.text, out);
    // `url` is not visible text, and a destination is not what this guard is
    // about — the media check below would otherwise match a URL containing
    // image markdown and degrade a perfectly good reply.
  }
  return out;
}

const textMatches = (value, predicate) => visibleStrings(value).some(predicate);
...
// the check sees the same values the reader will — and through the styled
// array form too, since a styled paragraph is where a leak would otherwise
// hide from a string-only test.
function someBlockText(blocks, predicate) {
  for (const b of blocks || []) {
    if (b?.text != null && textMatches(b.text, predicate)) return true;
    if (b?.caption?.text != null && textMatches(b.caption.text, predicate)) return true;
    if (typeof b?.summary === 'string' && predicate(b.summary)) return true;
    if (Array.isArray(b?.cells)) {
      for (const row of b.cells) {
        for (const cell of row || []) {
          if (cell?.text != null && textMatches(cell.text, predicate)) return true;
        }
      }
    }
    ...
    if (Array.isArray(b?.items)) {
      for (const item of b.items) {
        if (item?.text != null && textMatches(item.text, predicate)) return true;
 ...
 *   verdict, re-read per reply. False → render flat, exactly as before styling
 *   existed.
 * @param {() => void} [deps.onStylingRejected] — a styled send was refused and
 *   the flattened re-render of the SAME content landed. One variable changed
 *   between the two, so this is the only evidence about styling worth
 *   recording; the caller decides when enough of it is a verdict.
 * @param {() => void} [deps.onStylingAccepted] — a styled send landed
...
function createRichDeliveryFactory({
  ...
  return function makeDeliverText({ chatId, threadId, allowedRoots = null } = {}) {
    return async function deliverText({ text, replyToMessageId = null, meta = {} } = {}) {
      ...
      // Re-read per reply so a latch that trips mid-turn takes effect on the
      // next one, and so a chat is never stuck styling into a server that
      // has already refused it.
      let styled = isInlineStylingEnabled() === true;

      // `partial` is deliberately never set: final mode is the only one that
      // both resolves media and emits the complete tree.
      const render = (resolveMedia, inlineStyling = styled) => {
        try {
          const rendered = resolveMedia
            ? toRichBlocks(text, { resolveMedia, inlineStyling })
            : toRichBlocks(body, { inlineStyling });
          ...
          if (resolveMedia && someBlockText(rendered.blocks, carriesMediaMarkdown)) {
            const stripped = toRichBlocks(body, { inlineStyling });
      ...
      let retryBlocks = null;
      if (out && !out.wentRich && out.fallback === 'media-source-changed' && media) {
        retryBlocks = render(REJECT_ALL_MEDIA);
        if (retryBlocks) out = await attempt(retryBlocks);
      }

      // The server refused this payload and the only unusual thing in it is
      // the styling, so strip that before giving up the structure. Losing
      // every heading and table because one code span was unwelcome would be
      // the worse trade.
      //
      // The retry is also the experiment, which is why it flattens the tree
      // that was REFUSED rather than re-rendering the source: re-rendering
      // would resolve media a second time and could differ in ways nothing
      // here can see, and then a success would have changed two things. This
      // way the two payloads differ in exactly one respect, so
      // refused-then-accepted is evidence about styling and nothing else.
      const lastBlocks = out?.wentRich ? null : (retryBlocks ?? blocks);
      const contentRefused = out?.fallback === 'content-error' || out?.fallback === 'content-limit';
      if (styled && out && !out.wentRich && contentRefused && blocksAreStyled(lastBlocks)) {
        styled = false;
        const retried = await attempt(flattenStyledBlocks(lastBlocks));
        // Retry on either class — a flatter payload may well be what an
        // oversized reply needed — but only ONE of them is evidence.
        // Flattening removes styling AND shrinks the payload, so a refusal
        // that named a limit is explained by the shrink; counting it would
        // let two oversized replies disable styling for the process.
        if (retried?.wentRich && out.fallback === 'content-error') {
          try { onStylingRejected(); }
          catch (err) { logger?.error?.(`[rich-dispatch] styling verdict failed: ${redactError(err.message)}`); }
        }
        out = retried;
      }
      ...
      if (styled) {
        // A styled payload the server accepted — the evidence that ends any
        // run of suspicion, so two unrelated content errors far apart cannot
        // add up to a verdict.
        try { onStylingAccepted(); }
        catch (err) { logger?.error?.(`[rich-dispatch] styling verdict failed: ${redactError(err.message)}`); }
      }
