    ...
    return async function deliverText({ text, replyToMessageId = null, meta = {} } = {}) {
      ...
      const render = (resolveMedia) => {
        try {
          ...
          // or a parenthesis trips the former and is not an image to the
          // latter, so it yields no descriptor, survives as literal text, and
          // would put an absolute path on screen. macOS names every
          // screenshot that way. Fall back to the stripped body, which has no
          // such gap — nothing on this path may render a local path.
          if (resolveMedia && IMAGE_RE.test(JSON.stringify(rendered.blocks))) {
