...
function createForegroundCanaryAuthorizer({
  ...
  if (typeof getActiveHandlerCount !== 'function') {
    throw new TypeError('foreground canary active-handler count reader is required');
  }
  ...
  if (!Number.isSafeInteger(tokenLimit) || tokenLimit <= 0) {
    throw new TypeError('foreground canary token limit is invalid');
  }
  ...
  function inspectLiveTarget({ provider, scopeDigest }) {
    ...
    let active;
    try {
      activeCount = getActiveHandlerCount();
      active = getActiveHandlerTargets();
    } catch {
      return reject('target-unavailable');
    }
    if (activeCount !== 1 || !Array.isArray(active) || active.length !== 1) {
  ...
  function probe(request) {
    ...
    const issuedAt = now();
    for (const [requestId, stored] of tokens) {
      if (issuedAt > stored.expiresAt) tokens.delete(requestId);
    }
    if (!tokens.has(request.id) && tokens.size >= tokenLimit) {
      return {
        outcome: 'rejected',
