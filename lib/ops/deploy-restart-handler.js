...
function createDeployRestartHandler({
  ...
  if (typeof persistForegroundCanaryAuthorization !== 'function') {
    throw new TypeError('foreground canary authorization persistence is required');
  }
  ...
  return function requestDeployRestart(request) {
    const oldPid = getPid();
    const isForegroundRequest = request != null
      && typeof request === 'object'
      && Object.hasOwn(request, 'foreground_expectation');
    if (getIsShuttingDown() && !isForegroundRequest) {
    ...
    if (foregroundExpectation !== undefined) {
      ...
      try {
        persistForegroundCanaryAuthorization(authorization.authorizationEvent);
      } catch {
        return rejected(
          oldPid,
          restartRequestId,
