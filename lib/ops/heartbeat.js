 ...
 * Wiring the heartbeat file into an actual Netdata alert (netdata_watch_units /
...
function createHeartbeat({ dataDir, botName, authDisabledGate, intervalMs = 60_000, now = Date.now }) {
  ...
  // The name lands in a path segment. Reject anything unsafe rather than rewriting
  // it: sanitising would map two distinct names ('a/b' and 'a_b') onto one file and
  // silently restore the very collision this split exists to prevent.
  if (!/^[a-zA-Z0-9._-]+$/.test(botName) || botName === '.' || botName === '..') {
    throw new TypeError(`createHeartbeat: unsafe botName ${JSON.stringify(botName)} — allowed: A-Z a-z 0-9 . _ -`);
  }
