...
function processMatchesRuntime(proc, runtime, backend) {
...
function resolveCodexEnabled(config, chatId, threadId = null) {
  return resolveBoolOverride(
    config,
    'codexEnabled',
    chatId,
    threadId,
...
function requireCodexEnabled(config, chatId, threadId = null) {
  if (resolveCodexEnabled(config, chatId, threadId)) return true;
  throw new RuntimeConfigError(
    'Codex is not enabled for this chat.',
    {
      code: 'CODEX_SCOPE_DISABLED',
      runtime: 'codex',
      reason: 'effective codexEnabled policy is false',
      action: 'Enable Codex for this chat before selecting or dispatching it.',
    },
  );
}

function requireCodexDispatchEnabled({
  config,
  chatId,
  threadId = null,
  selectedProvider,
  ...
  if (selectedProvider !== 'codex' && !liveCodexGeneration) return false;
  return requireCodexEnabled(config, chatId, threadId);
}
...
function resolveRuntimeConfig({
  ...
  if (isCodex) {
    requireCodexEnabled(config, chatId, threadId);
...
/**
 * Resolve the prospective Codex inputs for one chat/topic without consulting
 * its currently selected runtime. This supports preflight before an atomic
 * runtime switch; it does not make Codex available and carries no receipt.
 */
function resolveCodexRuntimeCandidate({
  config = {},
  chatId = null,
  threadId = null,
} = {}) {
...
function resolveRuntimeDescriptor({
  ...
  const canonicalBackend = isCodex
    ...
    : pickedBackend === 'cli'
      ? 'cli'
      : 'sdk';
  const codexEnabled = resolveCodexEnabled(config, chatId, threadId);
  return Object.freeze({
    configuredPm: pmChoice.value,
