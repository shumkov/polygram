...
function createStore(rawDb, now = () => Date.now()) {
  ...
  return {
    issue({
      ...
      // The row is an audit + display copy: the card renders it and the digest
      // dedups repeated hook fires. The tool itself runs on the caller's own
      // in-memory input, so masking recognized credentials here removes them
      // from durable storage without touching what gets executed. Digesting
      // the masked form also stops the digest from being a fingerprint of the
      // credential, while staying deterministic for dedupe.
      // Sanitize the STRUCTURE before serializing: a value the serializer
      // escapes no longer looks like a declared credential to the detector,
      // masking the serialized form can splice across its delimiters, and a
      // credential named only by its key (`{"password":"…"}`) is invisible to
      // a text pass. A caller that hands over an already-serialized document
      // gets the same treatment — it is parsed back first, and only genuinely
      // non-JSON input falls back to text masking.
      let tool_input_json;
      let sanitized;
      if (typeof tool_input === 'string') {
        const res = sanitizeDurableJsonText(tool_input);
        tool_input_json = res.text;
        sanitized = res.changed;
      } else {
        const source = tool_input || {};
        tool_input_json = JSON.stringify(sanitizeDurableStructured(source));
        sanitized = tool_input_json !== JSON.stringify(source);
      }
      // The legacy dedupe key is (turn, digest of the stored input), which is
      // only sound while the stored input distinguishes two calls. Once a
      // credential is masked it no longer does: two different secrets mask to
      // the same text, so a shared digest would let one operator decision
      // authorize a different command. A sanitized row therefore gets a
      // random per-row identity — unique, and derived from nothing — and skips
      // legacy reuse entirely. Dedupe by the provider's stable tool_use_id is
      // unaffected: that key never depended on the input.
      const tool_input_digest = sanitized
        ? crypto.randomBytes(8).toString('hex')
        : digestInput(tool_input_json);
      ...
      const tx = rawDb.transaction(() => {
        let existing = null;
        if (tool_use_id) {
          existing = findDedupByToolUseIdStmt.get(bot_name, tool_use_id);
        } else if (!sanitized) {
          existing = findDedupStmt.get(bot_name, turn_id, tool_input_digest);
        }
