 ...
 *
 * canUseTool is an SDK in-process callback. When a gated tool fires,
 * polygram posts a Telegram inline-keyboard card to the admin chat
 * and PARKS a Promise that resolves on user click. The SDK awaits
 * that Promise — so the in-flight tool sleeps until the user decides.
 *
 * NOTE: as of 0.9.0 commit 12, polygram.js still uses an inline
 * `Map` for this purpose; this factory is the structured-replacement
 * target for the polygram.js extraction (Phase B commit 19, see
 * `docs/0.9.0-structural-reorg-plan.md`). Wiring it brings the
 * 200-waiter cap into production runtime; today's inline Map is
 * unbounded.
 *
