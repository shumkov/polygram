...
class ChannelsProcess extends Process {
  ...
  async _pollMidTurnDialogs() {
    ...
    for (const prompt of MID_TURN_PROMPTS) {
      ...
      this.logger.warn?.(
        `[${this.label}] channels: mid-turn dialog detected name=${prompt.name} ` +
      ...
      this._logEvent('channels-mid-turn-dialog-detected', {
        name: prompt.name,
        action: prompt.action,
      ...
      if (prompt.action === 'enter') {
        try {
          ...
          this.logger.warn?.(
            `[${this.label}] channels: mid-turn dismiss-Enter failed for ${prompt.name}: ${err.message}`,
          );
        }
      }
