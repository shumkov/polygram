...
class ChannelsProcess extends Process {
    ...
    return new Promise((resolve, reject) => {
      const pending = {
        ...
        hardTimer: setTimeout(() => {
          ...
          // Step E: emit 'idle' so reaction-cyclers (HeartbeatReactor + the
          // polygram per-msg reactor) stop. Without this, the reactor stays
          // cycling forever because turn-timeout takes the reject() path
          // instead of _resolveTurn (which is the only other path that emits
          // idle).
          this.emit('idle');
  ...
  async resetSession({ reason = 'reset' } = {}) {
    ...
    // ChannelsProcess would keep cycling until the next 'thinking' (next send)
  ...
  async respondToPermission(requestId, behavior) {
    ...
    // 'approval-required' emit time is bound to THIS ChannelsProcess. If the
    // user taps Approve/Deny 11+ min later, the original turn may have timed
    // out, the proc killed, and a new ChannelsProcess spawned for the same
