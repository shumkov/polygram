 ...
 *                           subscription billing; canonical post-0.12)
 *   - 'tmux' → TmuxProcess (legacy; deletion in Phase 4; still alive in 0.12 for soak)
 *
 * Config aliases (back-compat for existing chat configs):
 *   - 'channels' → 'cli' (0.11.0-channels driver folded into CliProcess)
 *
 * Phase 4.5+ will additionally alias 'tmux' → 'cli' once TmuxProcess deletes.
 *
...
const ALIASES = new Map([
  ['channels', 'cli'],
  // Phase 4.5 will add: ['tmux', 'cli'] once TmuxProcess deletes.
]);
 ...
 * @param {object} [opts.tmuxRunner]       — required when ANY chat routes to 'tmux' or 'cli'
 * @param {string} [opts.botName]          — required when ANY chat routes to 'tmux' or 'cli'
 ...
 */
const CANONICAL_BACKENDS = new Set(['sdk', 'tmux', 'cli']);
...
// tracking so unit tests can verify alias-warn behavior across multiple
// pickBackend() invocations within a single test run.
