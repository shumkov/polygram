...
class TmuxProcess extends Process {
  ...
  constructor({
    ...
    // Autosteer turns the TUI has parked in its input queue
    // (`queue-operation enqueue`), oldest first. A later
    // `queue-operation remove` folds the oldest into the running turn
    // (no user-message follows — it must be resolved from the remove);
    // a `dequeue` releases it to run as a fresh turn (its
    // user-message follows and _routeUserMessage handles it).
    this._enqueuedAutosteers = [];
  ...
  async _runTurn(turn) {
    ...
    try {
      ...
      // hard turn timeout. JSONL is primary (clean structured text);
      // capture-pane is the liveness fallback.
      ...
      if (winner.kind === 'jsonl') {
        ...
        if (turn.text) {
          ...
          if (late.kind === 'jsonl-late') {
            ...
            text = turn.text || this._extractTurnReply(captureAtStart, winner.cap);
          }
        }
      }
  ...
  /** Extract every correlation token present in a JSONL user-message. */
  _extractTokens(text) {
    if (typeof text !== 'string') return [];
    const m = text.match(/pgm-corr-[0-9a-f]+/g);
    return m ? [...new Set(m)] : [];
  }
      ...
      if (ev.operation === 'enqueue' && ev.content) {
        ...
        for (const tok of tokens) {
          const turn = this._ledger.find(
            (t) => t.token === tok && t.kind === 'autosteer'
              && t.state !== 'done' && t.state !== 'failed',
          );
          if (turn && !this._enqueuedAutosteers.includes(turn)) {
            this._enqueuedAutosteers.push(turn);
          }
        }
      } else if (ev.operation === 'remove') {
        const turn = this._enqueuedAutosteers.shift();
        if (turn && turn.state !== 'done' && turn.state !== 'failed') {
          this._foldAutosteer(turn);
        }
      } else if (ev.operation === 'dequeue') {
        // Released as a fresh turn — drop our enqueue tracking; the
        // user-message that follows is routed by _routeUserMessage.
        this._enqueuedAutosteers.shift();
      }
