...
class TmuxProcess extends Process {
  ...
  async send(prompt, opts = {}) {
    ...
    this._turnState = {
      ...
      // L5: store the primary's pasted prompt (oneLine form, same
      // transform as pasteText applies) so the user-message handler
      // can distinguish "primary's own prompt arrived in JSONL"
      // (text === pastedPrompt or contains it) vs "a queue dequeue
      // arrived" (text doesn't match). The L5 fix avoids
      // mis-attributing the primary's reply to an autosteered msg
      // when the TUI concatenates pastes into one user-message.
      pastedPromptOneLine: String(prompt || '').replace(/[\x00-\x08\x0b-\x1f\x7f]/g, '').replace(/\r?\n/g, ' / '),
    };
    ...
    try {
      ...
      // L1 deep fix: if capture-pane won but the turn has used a
      // tool (rc.11 set toolUsedThisTurn on the first tool_use
      // result), the agent is still working — the "ready" hint
      // was a transient idle between tool calls, NOT the actual
      // end of the turn. Discard the capture win and wait for the
      // real end_turn result event from JSONL.
      if (winner.kind === 'capture' && this._turnState?.toolUsedThisTurn) {
        winner = { kind: 'jsonl', ev: await turnResultP };
      }
      ...
      if (winner.kind === 'jsonl') {
        ...
        // 0.10.0 Phase 1: capture-pane quiescence judged the turn
        // complete. The JSONL aggregator may still be holding the
        // final assistant message buffered (it fires `result` only
        // when a trailing line finalizes the message.id). Force a
        // flush so the buffered `result` — with clean structured
        // text — settles `turnResultP` now, instead of losing the
        // race to the capture-pane diff.
  ...
  _completeTurn() {
    this.inFlight = false;
    // 0.10.0 Phase 1: finalize any assistant message still buffered in
    // the JSONL aggregator BEFORE clearing _turnState. Without this, a
    // turn that ended without a terminal `result` (e.g. turnTimeoutMs
    // fired) would leave the buffer holding this turn's message; the
    // NEXT turn's first JSONL line would then finalize it and the
    // stale `result` would mis-resolve turn N+1. Flushing here lands
    // any straggler `result` against THIS turn's (already-resolved)
    // _turnState — a harmless no-op — and guarantees the aggregator
    // starts the next turn empty.
    this._sessionLogTail?.flushParser?.();
  ...
  _handleSessionEvent(ev) {
    if (ev.type === 'assistant-chunk') {
      // rc.12.1: while _extraTurnState is set, ALL chunks belong to
      // turn 2 (autosteer NEW-TURN extraction). The TUI processes
      // one turn at a time; if a new pm.send arrived during turn 2,
      // its paste is queued in the TUI behind turn 2 — JSONL events
      // flowing while _extraTurnState is unresolved are turn 2's,
      // never turn 3's. Pre-rc.12.1 _turnState was checked first
      // and stole turn 2's chunks → extra-turn-reply never fired
      // → typing loop leaked. Ivan caught this on shumorobot
      // 2026-05-15 (events 1575-1578).
      if (this._extraTurnState) {
        this._extraTurnState.text = this._extraTurnState.text
          ? `${this._extraTurnState.text}\n\n${ev.text}`
          : ev.text;
      } else if (this._turnState) {
      ...
      if (this._turnState) {
        this._turnState.toolUses++;
        // 0.10.0 Phase 1: the JSONL aggregator now defers `result`
        // until a message finalizes, so the `result{subtype:tool_use}`
        // that used to set this flag can arrive later than the
        // capture-pane race. The `tool-use` event itself is the
        // earliest, most direct proof a tool ran — set the flag here
        // so the capture-pane win cannot resolve a still-working turn.
        this._turnState.toolUsedThisTurn = true;
      }
      ...
      // rc.11: ONLY resolve the turn on a TERMINAL stop_reason. The
      // JSONL parser synthesizes a 'result' event for every assistant
      // message with a stop_reason — including 'tool_use', which is
      // NON-terminal (the agent paused to run a tool; more assistant
      // messages will follow). Pre-rc.11, pm.send resolved on the
      // first 'tool_use' result → polygram treated it as a
      // tool-only-completion → cleared reactions while the agent
      // was still working. The final reply then arrived via
      // autonomous-assistant-message (since _turnState was cleared
      // by _completeTurn) seconds later, disconnected from the
      // user's input. Ivan caught this on shumorobot 2026-05-15
      // (msg 681 "how many folders" → tools ran → reply landed 14s
      // later via autonomous-wakeup).
      //
      // Terminal stop_reasons: 'end_turn' (mapped to subtype:'success'
      // by the parser), 'max_tokens', 'stop_sequence', 'refusal'.
      // Non-terminal: 'tool_use' — agent will continue after the
      // tool result arrives.
      if (ev.subtype === 'tool_use') {
        // Track tool-use count is already done via the 'tool-use'
        // event branch above; nothing to do here. Critically: do
        // NOT resolve _turnState — wait for the terminal result.
        // L1 deep fix: mark the current turn as "tool-used" so the
        // capture-pane race (which can see transient ready between
        // tool_result and the next tool_use OR the final text)
        // cannot win — only end_turn resolves now. Otherwise
        // multi-tool sequences sometimes pm.send-return with the
        // empty text from a brief idle window before the agent
        // emits final text.
        if (this._turnState) this._turnState.toolUsedThisTurn = true;
        return;
      }
      // rc.12.1: same priority swap as assistant-chunk above —
      // _extraTurnState must drain BEFORE _turnState because turn 2
      // (autosteer NEW-TURN) started in the TUI before any new
      // primary pm.send. Pre-rc.12.1, _turnState resolved on the
      // first terminal result (which was msg 692's, intended for
      // _extraTurnState) — msg 694's primary turn captured the
      // wrong reply, _extraTurnState stayed unresolved, typing
      // loop never stopped.
      // rc.12.1: extra-turn drains first. Same rationale as the
      // assistant-chunk branch above — turn 2 owns events until
      ...
      if (idx >= 0) {
        ...
        // rc.11.1: observability — signal which path was taken so the
        // events table records autosteer resolution. Without this,
        // diagnosing future autosteer regressions requires manual
        // capture-pane inspection.
        ...
        // queue-folded arrived but no matching pending autosteer.
        // This is a CONTENT-MATCH FAILURE — would have caught the
        // rc.11.1 regression immediately had this event existed.
        this.emit('autosteer-match-miss', {
          phase: 'queue-folded',
          prompt_head: (ev.prompt || '').slice(0, 80),
      ...
      // L5 fix: when a user-message ALSO contains the primary turn's
      // own prompt content, the TUI concatenated polygram's pm.send
      // paste with one or more injectUserMessage pastes into a single
      // user input. The agent's reply addresses all of them, so it
      // belongs to the primary turn — autosteers whose content
      // appears in this user-message were effectively folded at the
      // input level. Pop them with via='fold' (no extra-turn-reply
      // fires; primary's reply covers them).
      //
      // The discriminator is content-based: if ev.text contains the
      // primary's pasted prompt, this IS the primary's user-message
      // (possibly with autosteered content concatenated in). If
      // ev.text does NOT contain the primary's prompt, this is a
      // queue dequeue → match as new-turn.
      const primaryPrompt = this._turnState?.pastedPromptOneLine;
      const isPrimaryOwnPrompt = primaryPrompt
        && primaryPrompt.length > 0
        && ev.text.includes(primaryPrompt);
      if (isPrimaryOwnPrompt && matchedIdxs.length > 0) {
        // Primary's own user-message with autosteer(s) concatenated.
        // Pop the pendings as folds.
        for (let i = matchedIdxs.length - 1; i >= 0; i--) {
          const m = this._pendingAutosteers[matchedIdxs[i]];
      ...
      } else if (isPrimaryOwnPrompt) {
        // Primary's own user-message with no autosteer concatenation.
        // No matches to process; nothing to do. (Just don't fall
        // into the match-miss branch below since this user-message
        // legitimately belongs to the primary, not an autosteer.)
      } else if (matchedIdxs.length > 0) {
      ...
      // L2 fix (spike leftover): `last-prompt` fires in JSONL when
      // a user prompt is REGISTERED, BEFORE the agent's reply.
      // Confirmed in claude v2.1.142 — turn 3 of three-sequential-
      // sends emitted last-prompt for the prompt as the FIRST
      // event after pasting, then the agent's assistant reply
      // would arrive later. Pre-L2 logic synthesised a "success"
      // result here with _turnState.text='' (no chunks yet),
      // resolving pm.send with empty text before the model
      // produced its reply.
      //
      // Only fire the synthetic safety-net when we ALREADY have
      // accumulated text. That preserves the original intent
      // (rescue a turn whose end_turn never wrote) without
      // hijacking turns that haven't started replying yet.
      // If a turn ends with no text at all AND no end_turn ever
      // fires (extremely rare), it'll time out via turnTimeoutMs
      // instead — which is correct behaviour for a stalled turn.
      if (this._turnState
        && this._turnState.resolveResult
        && !this._turnState.resultEvent
        && this._turnState.text
        && this._turnState.text.length > 0) {
        const synthetic = {
          type: 'result',
          subtype: 'success',
          text: this._turnState.text,
  ...
  _extractTurnReply(captureAtStart, captureAtEnd) {
    ...
    if (captureAtStart && captureAtEnd.startsWith(captureAtStart)) {
      extracted = captureAtEnd.slice(captureAtStart.length).trim();
    } else {
      // Fallback: return whatever's after the user's last prompt marker.
      extracted = captureAtEnd.trim();
    }
    // L1 defense-in-depth: if banner survived the diff, refuse to
    // return it as the turn's reply. Should never happen now that
    // _awaitTurnComplete gates on TUI_BANNER_RE, but keep this as
    // a final guardrail so a future regression in the poll loop
    // can't cause polygram to deliver the banner to Telegram.
    if (TUI_BANNER_RE.test(extracted)) return '';
    return extracted;
  }
  ...
  injectUserMessage({ content, priority = 'next', shouldQuery, msgId } = {}) {
    ...
    // rc.11.1: compute the post-paste oneLine form for content
    // matching against JSONL queue-operation/user events. The runner's
    // pasteText normalises `\r?\n` → MULTILINE_SEPARATOR (' / ')
    // before pasting (tmux-runner.js:222). The TUI then records that
    // post-paste form in JSONL's queue-operation enqueue and the
    // later top-level user message. Storing the pre-paste `safe`
    // form (with newlines) in _pendingAutosteers caused a permanent
    // content mismatch — `a.content` had `\n`, `ev.text` had ` / ` —
    // so the NEW-TURN extra-turn-started/reply path never fired for
    // any multi-line prompt (which is every polygram inbound, since
    // they're all wrapped with `<polygram-info>` blocks). Ivan
    // caught the visible symptom on shumorobot 2026-05-15 (msg 685
    // "ok!" → turn 1 short → autosteered msg 686 dequeued as turn
    // 2 → reply landed via autonomous-wakeup-message instead of the
    ...
    if (msgId != null) {
      this._pendingAutosteers.push({ content: oneLine, msgId });
    }
