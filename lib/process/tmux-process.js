...
class TmuxProcess extends Process {
  ...
  async _waitForReady() {
    ...
    // several MCP servers) renders `? for shortcuts` at the BOTTOM of
    // its still-visible startup banner immediately — before the
    // agent/MCP servers have settled. The old `_waitForReady` matched
    // READY_HINTS_RE and returned AT ONCE, so `start()` resolved and
    // the first `send()` pasted the prompt into a TUI mid-startup;
    // the submitted Enter was dropped → the prompt sat unsubmitted →
    // the turn never began (a fake THINKING→STALL followed).
    //
    // `_awaitTurnComplete` already banner-gates the ready hint (a
    // ready hint under a visible banner = startup artifact, not turn
    // completion — the "L1 fix"); `_waitForReady` did NOT. That
    // asymmetry WAS the bug. Fix: ignore the ready hint while the
    // banner box-drawing chars are still on the pane bottom — once
    // the agent/MCP startup output has scrolled the banner away the
    // hint becomes a real ready signal. Bounded by `readyTimeoutMs`,
    // so a genuinely wedged spawn (banner never clears) still throws
    // TMUX_READY_TIMEOUT — the gate cannot mask a real hang.
    ...
    try {
      while (this._now() < deadline) {
        ...
        // L1/B6 banner gate: the ready hint only counts when the
        // startup banner is no longer on the pane bottom. Check the
        // last ~2000 chars (~10-20 lines) — mirrors the `bottomTail`
        // check in `_awaitTurnComplete`.
        const bottomTail = lastBuf.slice(-2000);
        if (READY_HINTS_RE.test(lastBuf) && !TUI_BANNER_RE.test(bottomTail)) {
          return;
        }
