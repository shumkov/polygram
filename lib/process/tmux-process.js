...
class TmuxProcess extends Process {
  ...
  _handleSessionEvent(ev) {
    if (ev.type === 'assistant-chunk') {
      ...
      // The live queue-activity signal. `enqueue` carries the pasted
      // content (with its embedded token) — but the authoritative
      // fold/new-turn routing happens when the dequeued `user-message`
      // lands and its token is matched. No action needed here in
      // Phase 2; Phase 3 uses enqueue for paste-gating.
