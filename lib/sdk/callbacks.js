  ...
  return {
    onInit: (sessionKey, event, entry) => {
      ...
      if (isCodexProcess(entry)) {
        // Codex init does not carry the settings admitted for a turn. The
        // request-prepared checkpoint persists that exact provider identity
        // atomically with the turn attempt before any app-server write.
        return;
      }
    ...
    onClose: (sessionKey, code, closeDetailOrEntry, appendedEntry) => {
      ...
      deliveryBarrier?.retireSession?.(sessionKey);
    },
    ...
    onHookEvent: (sessionKey, payload, entry) => {
      try {
        ...
        // `parse-error` and `unknown` used to carry their raw body so soak
        // analysis could tell transport corruption from schema drift. The
        // body is hook payload — tool inputs and assistant text — so what is
        // recorded now is its shape: an error code where the parser gave one,
        // and the two sizes. A body that is absent, empty or huge is still
        // distinguishable; its contents are not persisted.
        if (payload?.type === 'parse-error' || payload?.type === 'unknown') {
          let rawStr;
          try { rawStr = JSON.stringify(payload.raw); }
          catch { rawStr = String(payload.raw); }
          detail.raw_len = (rawStr || '').length;
          const parseError = payload?.error;
          detail.parse_error_code = parseError?.code || parseError?.name || null;
          detail.parse_error_len = typeof parseError === 'string'
            ? parseError.length
            : (parseError?.message?.length ?? 0);
        }
    ...
    onHookTailError: (sessionKey, payload /* , entry */) => {
      try {
        logEvent('hook-tail-error', {
          ...
          error_len:         (payload?.message || '').length,
          claude_session_id: payload?.sessionId ?? null,
        });
      } catch (err) {
