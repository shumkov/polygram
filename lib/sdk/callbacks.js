...
const { processAndDeliverAgentText } = require('../telegram/process-agent-reply');
  ...
  return {
    ...
    onAutonomousAssistantMessage: (sessionKey, msg /* , entry */) => {
      try {
        ...
        // rc.51: pipeline runs via processAndDeliverAgentText (the
        // shared helper extracted in lib/telegram/process-agent-reply.js).
        // Same protections as the rc.50 inlined version; one
        // implementation now serves autonomous-wakeup, extra-turn-reply,
        // and auto-resume-reply.
        //
        // Pre-rc.50 fallback: any pipeline dep missing → raw send.
        // Kept for back-compat with older test harnesses that don't
        // wire the deps; production wires all five.
        ...
        processAndDeliverAgentText({
          ...
          bot, tg,
          chatId, threadId,
          replyToMessageId: null,            // autonomous: no inbound msg
    ...
    onExtraTurnReply: (sessionKey, payload /* , entry */) => {
      try {
        ...
        // rc.51: route agent text through the shared pipeline (parse
        // + sanitize + chunk + deliver + stickers + reactions). Pre-
        // rc.51 this was `tg(bot, 'sendMessage', { text: <raw> })`
        // which bypassed all four protections — same latent bug
        // class as the rc.50 autonomous-wakeup fix. Not yet seen in
        // production but inevitable once claude embeds a sticker tag
        // or canned string in an autosteered extra-turn reply.
        //
        // Extra-turn-reply HAS a target msg (msgId = the autosteered
        // user message) so [react:EMOJI] tags CAN apply — unlike the
        // autonomous-wakeup case.
        const haveFullPipeline = (typeof parseResponse === 'function')
          && (typeof sanitizeAssistantReply === 'function')
          && (typeof chunkMarkdownText === 'function')
          && (typeof deliverReplies === 'function');
        ...
        if (haveFullPipeline) {
          processAndDeliverAgentText({
            ...
            bot, tg,
            chatId, threadId,
            replyToMessageId: msgId,         // tmux autosteered user msg
            ...
            logger.error?.(`[${botName}] extra-turn-reply pipeline failed: ${err.message}`);
          });
        } else {
          // Pre-rc.51 raw-sendMessage fallback (back-compat for test
          // harnesses that don't wire pipeline deps).
          const params = {
            chat_id: chatId,
            text,
        ...
        logEvent('extra-turn-reply', {
          ...
          text_len: text.length,
          backend: payload?.backend || 'tmux',
          pipeline: haveFullPipeline ? 'full' : 'raw-fallback',
        });
      } catch (err) {
