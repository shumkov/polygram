...
// Names of event types pm forwards to operator callbacks.
...
class ProcessManager {
  ...
  async interrupt(sessionKey) {
    ...
    try { await p.interrupt(); return true; }
    catch (err) {
      if (err.code === 'UNSUPPORTED_OPERATION' || err.code === 'NOT_IMPLEMENTED_YET') return false;
      throw err;
    }
  }
  ...
  async setModel(sessionKey, model) {
    const p = this.procs.get(sessionKey);
    if (!p || p.closed) return false;
    try { return await p.setModel(model); }
    catch (err) {
      if (err.code === 'UNSUPPORTED_OPERATION' || err.code === 'NOT_IMPLEMENTED_YET') return false;
      throw err;
    }
  ...
  async applyFlagSettings(sessionKey, settings) {
    const p = this.procs.get(sessionKey);
    if (!p || p.closed) return false;
    try { return await p.applyFlagSettings(settings); }
    catch (err) {
      if (err.code === 'UNSUPPORTED_OPERATION' || err.code === 'NOT_IMPLEMENTED_YET') return false;
      throw err;
    }
  ...
  async setPermissionMode(sessionKey, mode) {
    const p = this.procs.get(sessionKey);
    if (!p || p.closed) return false;
    try { return await p.setPermissionMode(mode); }
    catch (err) {
      if (err.code === 'UNSUPPORTED_OPERATION' || err.code === 'NOT_IMPLEMENTED_YET') return false;
      throw err;
    }
  ...
  async resetSession(sessionKey, opts) {
    const p = this.procs.get(sessionKey);
    if (!p) return { closed: true, drainedPendings: 0 };
    try { return await p.resetSession(opts); }
    catch (err) {
      if (err.code === 'UNSUPPORTED_OPERATION' || err.code === 'NOT_IMPLEMENTED_YET') {
        // Best-effort fallback: drain + kill, treat as reset.
  ...
  async getContextUsage(sessionKey) {
    ...
    try { return await p.getContextUsage(); }
    catch (err) {
      if (err.code === 'UNSUPPORTED_OPERATION' || err.code === 'NOT_IMPLEMENTED_YET') return null;
      throw err;
    }
  }
