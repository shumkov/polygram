...
function resolvePromptBackend({ config, chatId, threadId = null }) {
  return pickBackend({
    ...
    threadId: threadId == null || threadId === '' ? null : String(threadId),
    pmDefault: 'sdk',
  });
}
 ...
 *
 * @param {{ stickerEmojis?: string[], backend?: 'sdk'|'cli' }} [opts]
 */
 ...
 * @param {'sdk'|'cli'} params.backend - delivery contract for the selected process backend
 */
