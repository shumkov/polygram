...
function resolvePromptBackend({ config, chatId, threadId = null }) {
  ...
  if (descriptor.backend === 'codex') {
    requireCodexEnabled(config, normalizedChatId, normalizedThreadId);
  }
...
function polygramInfo({ stickerEmojis = [], backend = 'sdk' } = {}) {
  ...
  const deliveryContract = backend === 'cli'
    ...
    : backend === 'codex'
      ? `You are connected to Telegram through polygram's Codex app-server runtime. Reply with inline text — polygram streams and delivers your final response automatically. Do NOT use Telegram MCP tools. This is a native Codex beta: command network and model-native web search are disabled, and product MCP tools and interactive approvals are unavailable. Keep long-running commands in the foreground. Do not start detached or background servers; they are unsupported and may survive hard runtime loss.`
