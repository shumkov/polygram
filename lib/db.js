...
function wrap(db) {
  ...
  function recordShutdown({ botName, now = Date.now(), since, clean }) {
  ...
  function claimCodexRetryReservation(input) {
    ...
    return db.transaction(() => {
      ...
      if (reservation.original_generation_id === generationId) {
        throw codexError(
          'Codex retry requires a new generation after reboot release',
      ...
      const released = db.prepare(`
        SELECT 1
          FROM codex_reboot_releases
         WHERE stable_host_id = ?
           AND incident_boot_session_id = ?
           AND released_boot_session_id = ?
      `).get(
        reservation.original_stable_host_id,
        reservation.original_boot_session_id,
        generation.boot_session_id,
      );
      if (!released) {
        throw codexError(
          'Codex retry requires a durable reboot release',
          'CODEX_RETRY_REBOOT_RELEASE_REQUIRED',
        );
      }
      const existing = db.prepare(`
        SELECT * FROM codex_turn_attempts WHERE attempt_id = ?
  ...
  return {
    ...
    recordCleanShutdown({ botName, now = Date.now(), since } = {}) {
      return recordShutdown({ botName, now, since, clean: true });
    },
