    ...
    getForegroundCanaryTarget({ botName, chatId, telegramMessageId }) {
      const row = db.prepare(`
        SELECT message.id AS message_id,
               message.chat_id,
               message.thread_id,
               CAST(message.msg_id AS TEXT) AS telegram_message_id,
          ...
          JOIN inbound_runtime_selections AS selection
           ...
           AND selection.telegram_message_id = CAST(message.msg_id AS TEXT)
         WHERE message.direction = 'in'
           AND message.bot_name = ?
           AND message.chat_id = ?
           AND CAST(message.msg_id AS TEXT) = ?
      `).get(String(botName), String(chatId), String(telegramMessageId));
      if (!row) return null;
      return {
