...
const {
  codexDispatchReservationId,
} = require('./codex/dispatch-reservation-id');
const {
...
function wrap(db) {
  ...
  const codexProviderIdentityFields = [
    'session_key',
    'namespace',
    'provider',
    'provider_session_id',
  ...
  function hasExactCodexProviderIdentity(actual, expected) {
    return actual != null && codexProviderIdentityFields.every(
  ...
  function assertCodexCheckpointOwner(generation, kind) {
    if (kind === 'request-prepared') return;
  ...
  function insertCodexPreparedCheckpoint({
    generation,
    generationId,
    attemptId,
    method,
    ...
    requireCodexAttempt(attemptId, generationId);
    db.prepare(`
  ...
  function recordCodexCheckpoint(input) {
    ...
    return db.transaction(() => {
      ...
      if (attempt) {
        ...
        if (hasExactCodexCheckpoint({
          ...
          const deferred = (
            ...
            && isCleanRetirementDelivery(attempt)
          );
          return {
            changes: 0,
            attemptId,
      ...
      if (kind === 'request-prepared') {
        ...
        return insertCodexPreparedCheckpoint({
          generation,
          generationId,
          attemptId,
          method,
        ...
        if (
          ...
          throw codexError(
            'Codex queued cancellation requires exact pre-write identity',
            'CODEX_CHECKPOINT_INPUT_INVALID',
          );
        }
        db.prepare(`
      ...
      if (kind === 'request-write-attempted') {
        ...
        deferred = deliveryFailed && isCleanRetirementDelivery(attempt);
        if (!deferred) {
  ...
  function recordCodexTurnStartPrepared({
    checkpoint,
    provider,
    ...
    if (
      ...
      throw codexError(
        'Codex turn preparation requires a turn/start prepared checkpoint',
        'CODEX_CHECKPOINT_INPUT_INVALID',
      );
    }
    const generationId = requiredCodexString(
    ...
    const stableHostId = requiredCodexString(
      checkpoint.stable_host_id ?? checkpoint.hostIdentity,
      'stable host identity',
    );
    const bootSessionId = requiredCodexString(
      checkpoint.boot_session_id ?? checkpoint.bootSessionIdentity,
      'boot-session identity',
    );
    const attemptId = requiredCodexString(
      checkpoint.attemptId ?? checkpoint.attempt_id,
      'attempt ID',
    );
    const threadId = requiredCodexString(
      checkpoint.threadId ?? checkpoint.thread_id,
      'thread ID',
    );
    const sourceId = optionalCodexString(
      checkpoint.source,
      'source message ID',
    );
    const clientUserMessageId = requiredCodexString(
      checkpoint.clientUserMessageId ?? checkpoint.client_user_message_id,
      'client user message ID',
    );
    const turnId = optionalCodexString(
      checkpoint.turnId ?? checkpoint.turn_id,
      'turn ID',
    );
    const requestId = optionalCodexRequestId(
      checkpoint.requestId ?? checkpoint.request_id,
    );
    const itemId = optionalCodexString(
      checkpoint.itemId ?? checkpoint.item_id,
      'item ID',
    );
    ...
    const detailJson = checkpointDetail(checkpoint);
    const expectedLineage = expectedProviderGenerationId == null
      ? null
      : requiredCodexString(
        expectedProviderGenerationId,
        'provider-session lineage',
      );
    const providerSession = providerSessionParams({
      ...provider,
      session_key: requiredCodexString(provider?.session_key, 'session key'),
      cwd: requiredCodexString(provider?.cwd, 'provider cwd'),
      model: requiredCodexString(provider?.model, 'provider model'),
      effort: requiredCodexString(provider?.effort, 'provider effort'),
      spawn_profile_id: requiredCodexString(
        provider?.spawn_profile_id,
        'provider spawn profile ID',
      ),
    });
    if (
      providerSession.namespace !== CODEX_APP_SERVER_NAMESPACE
      || providerSession.provider !== 'codex'
      || providerSession.pm_backend !== 'codex'
      || providerSession.provider_session_id !== threadId
      || providerSession.agent !== null
    ) {
      throw codexError(
        'Codex turn preparation provider identity is inconsistent',
        'CODEX_CHECKPOINT_INPUT_INVALID',
      );
    }
    ...
    return db.transaction(() => {
      ...
      assertCodexCheckpointOwner(generation, 'turn-start-prepared');
      if (
        generation.session_key !== providerSession.session_key
        || generation.thread_id !== threadId
        || !db.prepare(`
          SELECT 1
            FROM codex_attempt_checkpoints
           WHERE generation_id = ?
             AND kind = 'thread-initialized'
             AND thread_id = ?
           LIMIT 1
        `).get(generationId, threadId)
      ) {
        throw codexError(
          'Codex turn preparation does not match the initialized generation',
          'CODEX_CHECKPOINT_STALE_GENERATION',
        );
      }

      const currentProvider = getProviderSessionStmt.get(
        providerSession.session_key,
        CODEX_APP_SERVER_NAMESPACE,
      );
      const attempt = db.prepare(`
        SELECT * FROM codex_turn_attempts WHERE attempt_id = ?
      ...
      if (attempt) {
        ...
        const exactCheckpoint = hasExactCodexCheckpoint({
          generationId,
          attemptId,
          kind: 'request-prepared',
          threadId,
        ...
        if (
          !hasExactCodexProviderIdentity(currentProvider, providerSession)
          || (
            expectedLineage !== null
            && currentProvider.generation_id !== expectedLineage
          )
        ) {
          throw codexError(
            'Codex prepared turn replay no longer matches provider identity',
            expectedLineage === null
              ? 'CODEX_TURN_PREPARATION_STALE'
              : 'CODEX_PROVIDER_LINEAGE_MISMATCH',
          );
        }
        return {
          changes: 0,
      ...
      if (
        ...
        throw codexError(
          'Codex strict resume provider lineage changed before turn preparation',
          'CODEX_PROVIDER_LINEAGE_MISMATCH',
        );
      }

      upsertProviderSessionStmt.run(providerSession);
      const checkpointResult = insertCodexPreparedCheckpoint({
        generation,
        generationId,
        attemptId,
        method: 'turn/start',
      ...
      if (expectedLineage !== null) {
        const committedProvider = getProviderSessionStmt.get(
          providerSession.session_key,
          CODEX_APP_SERVER_NAMESPACE,
        );
        if (
          !hasExactCodexProviderIdentity(committedProvider, providerSession)
          || committedProvider.generation_id !== expectedLineage
        ) {
          throw codexError(
            'Codex strict resume provider lineage rotated during turn preparation',
            'CODEX_PROVIDER_LINEAGE_MISMATCH',
          );
        }
      }
  ...
  function isCleanRetirementDelivery(attempt) {
    if (
      attempt?.method !== 'turn/start'
      || attempt.terminal_status !== 'interrupted'
      || !['terminal-pending', 'clean-pending'].includes(
        attempt.recovery_state,
      )
    ) return false;
    return db.prepare(`
  ...
  function prepareCodexCleanRetirement(input) {
    ...
    const providerSessionId = requiredCodexString(
      input?.provider_session_id,
      'provider session ID',
    );
    const providerTurnId = requiredCodexString(
      input?.provider_turn_id,
      'provider turn ID',
    );
    const sourceMessageId = requiredCodexString(
      input?.source_message_id == null
        ? input?.source_message_id
        : String(input.source_message_id),
      'source message ID',
    );
    const stableHostId = requiredCodexString(
      input?.stable_host_id,
    ...
    const ts = requiredTimestamp(
      input?.ts ?? Date.now(),
      'clean retirement preparation timestamp',
    );
    ...
    return db.transaction(() => {
      ...
      assertCodexCheckpointOwner(generation, 'clean-retirement-requested');
      const attempt = requireCodexAttempt(attemptId, generationId);
      const accepted = hasCodexTurnCheckpoint(
        generationId,
        attemptId,
        'turn-accepted',
        providerTurnId,
      ...
      if (
        ...
        || attempt.telegram_source_message_id !== sourceMessageId
        || attempt.delivery_state !== 'response-observed'
        || attempt.response_outcome !== 'result'
        || attempt.recovery_state !== 'active'
        || attempt.terminal_status != null
        ...
        throw codexError(
          'Codex clean retirement target is not the exact active accepted turn',
          'CODEX_RETIREMENT_PREPARATION_REJECTED',
        );
      }
      const existing = db.prepare(`
        SELECT thread_id, turn_id
         ...
         WHERE generation_id = ?
           AND attempt_id = ?
           AND kind = 'clean-retirement-requested'
      `).all(generationId, attemptId);
      if (existing.length > 0) {
        if (
          existing.length === 1
          && existing[0].thread_id === providerSessionId
          && existing[0].turn_id === providerTurnId
        ) {
          return {
            changes: 0,
        ...
        throw codexError(
          'Codex clean retirement target has conflicting ownership',
          'CODEX_RETIREMENT_PREPARATION_REJECTED',
        );
      }
      db.prepare(`
  ...
  function recordCodexDeliveryCheckpoint(input) {
    ...
    return db.transaction(() => {
      ...
      if (!input.retireGeneration || result.deferred === true) {
  ...
  function cleanRestartCancellationProof(attempt) {
    if (
      !attempt
      || attempt.delivery_state !== 'prepared'
      ...
      || attempt.turn_id != null
      || attempt.terminal_status != null
      || !['turn/start', 'queued/send'].includes(attempt.method)
      || typeof attempt.thread_id !== 'string'
      || attempt.thread_id.length === 0
      || typeof attempt.telegram_source_message_id !== 'string'
      || attempt.telegram_source_message_id.length === 0
      || typeof attempt.client_user_message_id !== 'string'
      || attempt.client_user_message_id.length === 0
    ) {
      return null;
    }
    const expectedKind = attempt.method === 'turn/start'
      ? 'active-start-cancelled'
      : 'queued-send-cancelled';
    const checkpoints = db.prepare(`
      SELECT kind, thread_id, turn_id, detail_json
        FROM codex_attempt_checkpoints
       WHERE generation_id = ?
         AND attempt_id = ?
       ORDER BY id
       LIMIT 3
    `).all(attempt.generation_id, attempt.attempt_id);
    const allowedKinds = attempt.method === 'turn/start'
      ? new Set(['request-prepared', expectedKind])
      : new Set([expectedKind]);
    if (
      checkpoints.some((checkpoint) => !allowedKinds.has(checkpoint.kind))
      || checkpoints.filter((checkpoint) => checkpoint.kind === expectedKind)
        .length !== 1
      || (
        attempt.method === 'turn/start'
        && checkpoints.filter((checkpoint) => checkpoint.kind === 'request-prepared')
          .length !== 1
      )
    ) {
      return null;
    }
    const cancellation = checkpoints.find(
      (checkpoint) => checkpoint.kind === expectedKind,
    );
    let detail;
    try {
      detail = JSON.parse(cancellation.detail_json);
    } catch {
      return null;
    }
    if (
      ...
      || detail.reason !== 'clean-restart'
      || cancellation.thread_id !== attempt.thread_id
      || cancellation.turn_id != null
      || detail.method !== attempt.method
      || detail.source !== attempt.telegram_source_message_id
      || detail.clientUserMessageId !== attempt.client_user_message_id
    ) {
      return null;
    }
    return {
  ...
  function getReplayProviderRecovery(input) {
    ...
    if (
      attempt
      && !['turn/start', 'turn/steer', 'queued/send'].includes(attempt.method)
    ) {
      return unknown('codex-evidence-conflict');
    }
    if (
      reservation
      && ['reserved', 'queue-authorized'].includes(reservation.state)
      && reservation.settled_ts != null
    ) {
      return unknown('codex-evidence-conflict');
    }
    ...
    if (linkedInput) {
      ...
      if (attempt.method === 'queued/send') {
        kind = 'queued-send';
      } else if (attempt.method !== 'turn/start') {
        return unknown('codex-evidence-conflict');
      } else {
        kind = 'primary-turn';
      }
    ...
    const cancellationProof = cleanRestartCancellationProof(attempt);
    ...
    return {
      ...
      reservation: reservation
        ? {
          ...
          steerAttemptId: reservation.steer_attempt_id,
  ...
  function prepareCodexCleanReplay(input = {}) {
    assertExactKeys(
      input,
      new Set([
        'source',
      ...
      'CODEX_CLEAN_REPLAY_PAYLOAD_REJECTED',
      'clean replay',
    );
    assertExactKeys(
      input.source,
      new Set(['botName', 'telegramChatId', 'telegramMessageId']),
      'CODEX_CLEAN_REPLAY_PAYLOAD_REJECTED',
      'clean replay source',
    );
    assertExactKeys(
      ...
      'CODEX_CLEAN_REPLAY_PAYLOAD_REJECTED',
      'clean replay generation',
    );
    assertExactKeys(
      input.owner,
      new Set(['stableHostId', 'bootSessionId']),
      'CODEX_CLEAN_REPLAY_PAYLOAD_REJECTED',
      'clean replay owner',
    );
    const sessionKey = requiredCodexString(
      input.sessionKey,
      'clean replay session key',
    );
    const botName = requiredCodexString(
      input.source?.botName,
      'clean replay bot name',
    );
    const telegramChatId = requiredCodexString(
      input.source?.telegramChatId,
      'clean replay Telegram chat ID',
    );
    const telegramMessageId = requiredCodexString(
      input.source?.telegramMessageId,
      'clean replay Telegram message ID',
    );
    const generationId = requiredCodexString(
      input.currentGeneration?.generationId,
      'clean replay generation ID',
    );
    const stableHostId = requiredCodexString(
      input.owner?.stableHostId,
      'clean replay stable host identity',
    );
    const bootSessionId = requiredCodexString(
      input.owner?.bootSessionId,
      'clean replay boot-session identity',
    );
    const ts = requiredTimestamp(
      input.ts ?? Date.now(),
      'clean replay timestamp',
    );
    const expectedEvidence = input.expectedEvidence;
    if (expectedEvidence == null || typeof expectedEvidence !== 'object') {
      throw codexError(
        'Codex clean replay expected evidence is required',
        'CODEX_CLEAN_REPLAY_EVIDENCE_INVALID',
      );
    }
    const reservationId = codexDispatchReservationId({
      botName,
      telegramChatId,
      telegramMessageId,
    });
    ...
    return db.transaction(() => {
      const actualEvidence = getReplayProviderRecovery({
        sessionKey,
        botName,
        telegramChatId,
        telegramMessageId,
      });
      const disposition = classifyCodexRecoveryEvidence(actualEvidence);
      if (
        !isDeepStrictEqual(actualEvidence, expectedEvidence)
        || disposition.action !== 'recover'
        || disposition.cleanRestartSafe !== true
      ) {
        throw codexError(
          'Codex clean replay evidence changed or is not safe',
          'CODEX_CLEAN_REPLAY_EVIDENCE_CHANGED',
        );
      }
      ...
      if (
        ...
        throw codexError(
          'Codex clean replay generation does not own the active lease',
          'CODEX_CLEAN_REPLAY_GENERATION_MISMATCH',
        );
      }

      const inboundRows = db.prepare(`
        SELECT id, handler_status
          FROM messages
         WHERE direction = 'in'
           AND bot_name = ?
           AND chat_id = ?
      ...
      if (
        ...
        || inboundRows[0].handler_status !== 'replay-pending'
      ) {
        throw codexError(
          'Codex clean replay no longer owns one replay-pending input',
          'CODEX_CLEAN_REPLAY_INBOUND_MISMATCH',
        );
      }
      ...
      if (reservationRows.length === 0) {
        ...
        if (
          ...
          throw codexError(
            'Codex clean replay reservation identity conflicts',
            'CODEX_CLEAN_REPLAY_RESERVATION_CONFLICT',
          );
        }
        const rebound = db.prepare(`
          UPDATE codex_dispatch_reservations
             SET generation_id = ?,
                 ...
                 settled_ts = NULL
           WHERE reservation_id = ?
        `).run(generationId, ts, reservationId);
        if (rebound.changes !== 1) {
          throw codexError(
            'Codex clean replay reservation could not be rebound',
            'CODEX_CLEAN_REPLAY_RESERVATION_CONFLICT',
          );
        }
      }
      ...
      const consumed = db.prepare(`
        UPDATE messages
         ...
         WHERE id = ?
           AND handler_status = 'replay-pending'
      `).run(inboundRows[0].id);
      if (consumed.changes !== 1) {
        throw codexError(
          'Codex clean replay input could not be consumed',
          'CODEX_CLEAN_REPLAY_INBOUND_MISMATCH',
        );
      }
      const reservation = readCodexDispatchReservation(reservationId);
      return {
        reservationId,
        generationId,
        reservation,
  ...
  return {
    ...
    prepareCodexCleanRetirement,
    recordCodexCheckpoint,
    recordCodexTurnStartPrepared,
