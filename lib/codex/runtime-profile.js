...
function renderOwnedConfig(config, {
  ...
  const hookTables = renderTrustTables(config);
  if (hookTables) lines.push(...hookTables.split('\n'));
  if (
...
function provisionBaseConfig(materialized) {
  const { configPath, rawConfig } = materialized;
...
async function verifyHookPhase(materialized, {
  binaryReceipt,
  ...
  const client = clientFactory({
    ...
    onNotification: async () => {
      throw fail(
        'Codex hook verification received an unexpected notification.',
        'CODEX_RUNTIME_POLICY_DRIFT',
        'Disable hooks and inspect the pinned app-server protocol drift.',
      );
    },
    onFault: (outcome) => {
  ...
  if (
    ...
    || typeof client.verifyHooks !== 'function'
    || typeof client.close !== 'function'
    || typeof client.waitForFault !== 'function'
  ) {
    throw new TypeError('Codex hook clientFactory returned an invalid client');
  }
  let result;
  let failure = null;
  try {
    await client.start();
    result = await client.verifyHooks({ phase });
  } catch (error) {
    failure = error;
  }
  try {
    ...
    throw fail(
      'Codex hook verification cleanup failed.',
      'CODEX_RUNTIME_CLIENT_CLOSE_FAILED',
      'Disable hooks and inspect the pinned app-server child cleanup.',
      error,
    );
  }
  let terminalFault;
  try {
    ...
    throw fail(
      'Codex hook verification fault handoff failed.',
      'CODEX_RUNTIME_CLIENT_FAULT',
      'Disable hooks and inspect the pinned app-server fault handoff.',
      error,
    );
  }
  if (latchedFault || terminalFault) {
    throw fail(
      'Codex hook verification client faulted.',
      'CODEX_RUNTIME_CLIENT_FAULT',
      'Disable hooks and inspect the pinned app-server fault.',
    );
  }
  if (failure) {
    if (failure instanceof CodexRuntimeProfileError) throw failure;
    throw fail(
      'Codex hook verification failed closed.',
      failure?.code === 'CODEX_HOOK_TRUST_UNVERIFIED'
        ? 'CODEX_HOOK_TRUST_UNVERIFIED'
        : 'CODEX_RUNTIME_POLICY_DRIFT',
      'Restore the exact owned hook manifest and runtime policy.',
      failure,
    );
  }
  return result;
...
async function characterizeRuntime(materialized, {
  ...
  try {
    await client.start();
    if (hookManifest != null) {
      if (typeof client.verifyHooks !== 'function') {
        throw new TypeError('Codex runtime client lacks hook verification');
      }
...
function createCodexRuntimeProfileBuilder({
  ...
  const hookProfiles = new WeakMap();
  const builder = {
    async prepare(options = {}) {
      ...
      let materialized = provisionRuntimeFiles(options, temporaryRoots);
      let hookMaterial = null;
      if (options.hooks == null) {
        provisionBaseConfig(materialized);
      } else {
        if (options.hooks.enabled === true) {
          ensurePrivateDirectory(
            path.join(materialized.workspace, CAPTURE_DIRECTORY),
            'CODEX_COMMAND_DIR_UNSAFE',
            'hook observation directory',
          );
        }
        const hookState = await prepareHookRuntime({
          hookOptions: options.hooks,
          codexHome: materialized.codexHome,
          workspace: materialized.workspace,
          baseOwnedConfig: materialized.ownedConfig,
          baseRawConfig: materialized.rawConfig,
          renderOwnedConfig: (ownedConfig) => renderOwnedConfig(ownedConfig, {
            codexHome: materialized.codexHome,
            workspace: materialized.workspace,
            daemonSecretRoots: materialized.daemonSecretRoots,
          }),
          verifyDiscovery: (hookManifest, expectedConfigSha256) => (
            verifyHookPhase(materialized, {
        ...
        hookMaterial = hookState.hookMaterial;
        materialized = {
          ...materialized,
          ownedConfig: hookState.ownedConfig,
          rawConfig: hookState.rawConfig,
          ownedConfigSha256: digest(hookState.rawConfig),
          hookManifest: hookState.hookManifest,
          hookArtifactsSha256: hookState.hookArtifactsSha256,
        };
      }
      const characterized = await characterizeRuntime(materialized, {
        ...
        sessionLauncherReceipt,
        clientFactory,
        hookManifest: materialized.hookManifest ?? null,
      });
      ...
      const profile = deepFreeze({
        ...
        ...(materialized.hookManifest == null ? {} : {
          hookManifest: materialized.hookManifest,
          hookArtifactsSha256: materialized.hookArtifactsSha256,
        }),
      });
      if (hookMaterial != null) {
        hookProfiles.set(profile, Object.freeze({
          hookOptions: Object.freeze({ ...options.hooks }),
          codexHome: materialized.codexHome,
          workspace: materialized.workspace,
        }));
      }
    ...
    reattest(profile) {
      if (profile?.hookManifest == null) return null;
      const binding = hookProfiles.get(profile);
      if (!binding) {
        throw fail(
          'Codex hook profile is not owned by this runtime builder.',
          'CODEX_OWNED_HOOKS_DRIFT',
          'Discard the stale profile and rerun exact preflight.',
        );
      }
      return reattestHookRuntime({
        ...binding,
        expected: profile,
      });
    },
