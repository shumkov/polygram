  ...
  for (const part of target.slice(root.length).split('/').filter(Boolean)) {
    ...
    if (
      ...
      throw fail(
        ...
        'CODEX_HOME_UNSAFE',
        `Move the Codex ${kind} under an owner-controlled path.`,
      );
    }
  }
}
...
function buildControlledEnv(codexHome, processEnv) {
  const home = requiredString(processEnv?.HOME, 'app-server HOME');
  const tmpdir = requiredString(processEnv?.TMPDIR, 'app-server TMPDIR');
  if (!path.isAbsolute(home) || !path.isAbsolute(tmpdir)) {
    throw fail(
      'Codex app-server HOME and TMPDIR must be absolute.',
      'CODEX_RUNTIME_PROFILE_INVALID',
      'Configure explicit absolute service HOME and TMPDIR values.',
    );
  }
...
function validateBinaryReceipt(receipt, schema) {
  if (
    !receipt
    || typeof receipt !== 'object'
    || !Object.isFrozen(receipt)
    || !path.isAbsolute(receipt.path ?? '')
    || receipt.version !== CODEX_CLI_PINNED_VERSION
    || receipt.sha256 !== CODEX_BINARY_SHA256
    || schema?.cliVersion !== CODEX_CLI_PINNED_VERSION
    || schema?.binarySha256 !== CODEX_BINARY_SHA256
...
function provisionRuntimeFiles(options, temporaryRoots) {
  ...
  if (existsSync(configPath)) {
    ...
    if (readFileSync(configPath, 'utf8') !== rawConfig) {
      throw fail(
        ...
        'Restore the exact owned config; it is never overwritten automatically.',
      );
    }
  } else {
    try {
  ...
  return {
    ...
    env: buildControlledEnv(codexHome, options.processEnv ?? process.env),
  };
}
...
function validateProjectedLayers(layers, materialized) {
  if (!Array.isArray(layers) || ![1, 2].includes(layers.length)) {
  ...
  for (const layer of layers) {
    ...
    if (layer.type === 'user') {
      userCount += 1;
      if (layer.configSha256 !== digest(materialized.ownedConfig)) {
        throw new Error('owned user layer hash changed');
      }
    } else if (layer.type === 'system') {
      systemCount += 1;
      if (layer.configSha256 !== digest({})) {
...
function createCodexRuntimeProfileBuilder({
  ...
  return Object.freeze({
    async prepare(options = {}) {
      validateBinaryReceipt(options.binaryReceipt, protocolSchema);
