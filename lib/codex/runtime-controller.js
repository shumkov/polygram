...
const { CODEX_APP_SERVER_NAMESPACE } = require('../db/sessions');
const {
  codexDispatchReservationId,
} = require('./dispatch-reservation-id');
...
function createCodexRuntimeController({
  ...
  async function prepareCodexRequest({ sessionKey, request, state }) {
    ...
    const hookConfigFingerprint = digest(configured.hooks);
    const existing = receiptsBySession.get(sessionKey);
    if (existing && sameProfileRequest(
      existing.receipt.expectedStaticProfile,
      request,
    ) && existing.hookConfigFingerprint === hookConfigFingerprint) {
      const catalogAgeMs = now() - existing.catalogPreparedAt;
      let currentCredentialState;
      try {
        if (existing.receipt.expectedStaticProfile.hookManifest != null) {
          if (typeof runtimeProfileBuilder.reattest !== 'function') {
            throw controllerError(
              'Codex hook profile cannot be re-attested.',
              'CODEX_OWNED_HOOKS_DRIFT',
              'Discard the profile and restore the hook-aware runtime builder.',
            );
          }
          runtimeProfileBuilder.reattest(
            existing.receipt.expectedStaticProfile,
          );
        }
    ...
    if (existing && receiptsBySession.get(sessionKey) === existing) {
      invalidatePreparedSession(sessionKey, existing);
    }
    ...
    const promise = (async () => {
      ...
      const expectedStaticProfile = await runtimeProfileBuilder.prepare({
        ...
        processEnv,
        sessionLauncher: configured.sessionLauncher,
        ...(configured.hooks == null ? {} : { hooks: configured.hooks }),
      });
      if (expectedStaticProfile.hookManifest != null) {
        if (typeof runtimeProfileBuilder.reattest !== 'function') {
          throw controllerError(
            'Codex hook profile cannot be re-attested.',
            'CODEX_OWNED_HOOKS_DRIFT',
        ...
        runtimeProfileBuilder.reattest(expectedStaticProfile);
      }
      const homeAttestation = await attestCodexHome(
      ...
      if (digest(deploymentConfig().hooks) !== hookConfigFingerprint) {
        throw controllerError(
          'Codex hook configuration changed during preflight.',
          'CODEX_PREFLIGHT_PROFILE_MISMATCH',
          'Retry after configuration changes settle.',
        );
      }
  ...
  function clientFactory({
    ...
    if (expectedStaticProfile.hookManifest != null) {
      if (typeof runtimeProfileBuilder.reattest !== 'function') {
        throw controllerError(
          'Codex hook profile cannot be re-attested before spawn.',
          'CODEX_OWNED_HOOKS_DRIFT',
          'Discard the profile and restore the hook-aware runtime builder.',
        );
      }
    ...
    return new orchestra.CodexAppServerClient({
      ...
      ...(expectedStaticProfile.hookManifest == null ? {} : {
        hookManifest: expectedStaticProfile.hookManifest,
      }),
  ...
  function registerProcess(proc, {
    ...
    const strictResume = (
      proc.spawnOptions?.resumePolicy === 'require-interrupted-turn'
    );
    if (
      (strictResume && (
        typeof expectedProviderGenerationId !== 'string'
        || expectedProviderGenerationId.length === 0
        || Buffer.byteLength(expectedProviderGenerationId) > 4096
      ))
      || (!strictResume && expectedProviderGenerationId !== null)
    ) {
      throw new TypeError(
        'Codex provider lineage is valid only for an exact strict resume',
      );
    }
  ...
  function settleQueuedDispatch({
    ...
    if (exactReservationId !== codexDispatchReservationId({
      botName: exactBotName,
      telegramChatId: exactChatId,
      telegramMessageId: exactMessageId,
    })) {
  ...
  function activateWithPreparedCheckpoint(record, payload) {
    if (
      ...
      || !['thread/start', 'thread/resume'].includes(payload.method)
    ) {
      throw controllerError(
  ...
  function persistPreparedTurnIdentity(record, payload, state) {
    const proc = record.proc;
    const pending = proc.current;
    const settings = pending?.admittedSettings;
    const sourceMsgId = pending?.context?.sourceMsgId ?? null;
    const starting = (
      proc.state === 'StartingTurn'
      && proc.settingsAdmissionClosed === false
    );
    const quiescing = (
    ...
    const reject = () => {
      throw controllerError(
        'Codex turn preparation does not match the exact admitted turn.',
        'CODEX_TURN_PREPARATION_REJECTED',
        'Fence the generation before retrying the user turn.',
      );
    };
    if (
      currentProcessBySession.get(proc.sessionKey) !== record
      || record.closed
      || record.retired
      || record.durabilityBlocked
      || record.retirementVerificationFailed
      ...
      || proc.runtime !== 'codex'
      || proc.backend !== 'codex'
      || proc.generationId !== payload.generationId
      || proc.hostIdentity !== state.identity.stableHostId
      || proc.bootSessionIdentity !== state.identity.bootSessionId
      || (!starting && !quiescing)
      || !pending
      || proc.pendingQueue?.[0] !== pending
      || pending.settled !== false
      || pending.startDeliveryState !== 'preparing'
      || pending.attemptId !== payload.attemptId
      || typeof pending.clientUserMessageId !== 'string'
      || pending.clientUserMessageId.length === 0
      || (pending.clientUserMessageId ?? null)
        !== (payload.clientUserMessageId ?? null)
      || sourceMsgId !== (payload.source ?? null)
      || pending.admittedSettings !== proc.admittingTurnSettings
      || !Object.isFrozen(settings)
      || typeof settings?.model !== 'string'
      || settings.model.length === 0
      || typeof settings?.effort !== 'string'
      || settings.effort.length === 0
      || typeof proc.providerSessionId !== 'string'
      || proc.providerSessionId.length === 0
      || proc.providerSessionId !== payload.threadId
      || proc.spawnProfileId !== record.receipt.spawnProfileId
      || proc.cwd !== record.receipt.expectedStaticProfile.cwd
    ) {
      reject();
    }
    const expectedProviderGenerationId = (
      record.expectedProviderGenerationId !== null
      && (
        record.strictResumeAttemptId === null
        || record.strictResumeAttemptId === payload.attemptId
      )
    )
      ? record.expectedProviderGenerationId
      : null;
    const result = db.recordCodexTurnStartPrepared({
      checkpoint: payload,
      provider: {
        ...
        pm_backend: 'codex',
        spawn_profile_id: proc.spawnProfileId,
  ...
  function checkpointSinkImpl(payload) {
    ...
    if (
      ...
      if (!record.durable) {
        throw controllerError(
          'Codex turn reached persistence before generation initialization.',
          'CODEX_GENERATION_NOT_DURABLE',
          'Quarantine the generation and inspect its initialization checkpoints.',
        );
      }
      persistPreparedTurnIdentity(record, payload, state);
  ...
  async function checkpointSink(payload) {
    try {
      return checkpointSinkImpl(payload);
    } catch (error) {
      if (
        payload?.kind === 'request-prepared'
        && payload?.method === 'turn/start'
