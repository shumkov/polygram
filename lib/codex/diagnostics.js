...
function leaseCheck(db) {
  ...
  if (lease.status === 'quarantined') {
    ...
    return check(
      'codex-lease',
      'fail',
      'daemon-wide Codex cleanup is unverified; restart the Polygram service after containment completes',
      {
        runtime: 'codex',
        scope: 'daemon',
        status: 'quarantined',
...
async function collectCodexDoctorChecks({
  ...
  checks.push(check(
    ...
    'warn',
    'detached and background servers are unsupported in the native beta; hard loss requires verified containment cleanup',
    {
      runtime: 'codex',
      detachedBackgroundSupported: false,
      hardLossRebootRequired: false,
      containmentCleanupRequired: true,
    },
  ));
  return checks;
}
