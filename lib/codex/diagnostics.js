...
function inspectCodexProfile(codexHome, profileId) {
...
function inspectCodexDenyRoot(codexHome, deniedRoot) {
  const configPath = path.join(codexHome, 'config.toml');
  const stat = lstatSync(configPath);
  if (stat.size > MAX_CONFIG_BYTES) {
    throw new Error('Codex owned config exceeded the inspection bound');
  ...
  const filesystemSection =
    `[permissions.${JSON.stringify(CODEX_PERMISSION_PROFILE_ID)}.filesystem]`;
...
function dependenciesWithDefaults(input = {}) {
  ...
  return {
    ...
    pinnedVersion:
      input.pinnedVersion ?? CODEX_CLI_PINNED_VERSION,
    pinnedBinarySha256:
      input.pinnedBinarySha256 ?? CODEX_BINARY_SHA256,
    profileId:
...
async function collectCodexDoctorChecks({
  ...
  try {
    ...
    if (
      receipt?.version !== dependencies.pinnedVersion
      || receipt?.sha256 !== dependencies.pinnedBinarySha256
    ) {
    ...
    checks.push(check(
      'codex-binary',
      'ok',
      `pinned runtime verified (${dependencies.pinnedVersion})`,
      {
        runtime: 'codex',
        pinned: true,
        version: dependencies.pinnedVersion,
      },
    ));
  } catch (error) {
    checks.push(check(
  ...
  if (
    schema?.cliVersion === dependencies.pinnedVersion
    && schema?.binarySha256 === dependencies.pinnedBinarySha256
  ...
  try {
    inspectCodexProfile(config.codex.home, dependencies.profileId);
    checks.push(check(
      'codex-profile',
      'ok',
  ...
  try {
    ...
    inspectCodexDenyRoot(config.codex.home, codexTmp);
    checks.push(check(
      'codex-tmpdir',
      'ok',
