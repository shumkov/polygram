...
async function buildCodexSpawnContext({
  ...
  if (strictResume) {
    const stored = db.getProviderSession(sessionKey, 'codex:app-server');
    const existingSessionId = validateStrictResumeSpawn({
      strictResume,
      promptBackend: 'codex',
      liveProcess,
      runtime: stored,
    ...
    return {
      ...
      expectedInterruptedTurnId: strictResume.expectedInterruptedTurnId,
