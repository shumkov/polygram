...
//
// SUBMIT_STUCK_RE matches the TUI's input prompt line (`❯`) still
// holding pasted content: either the bracketed-paste collapse marker
// "[Pasted text" or a polygram-prompt opening tag that polygram only
// ever sends as a paste body. An idle/submitted input box is a bare
// `❯` with nothing after it — it does NOT match. The match is
// anchored to the `❯` line so the SAME marker appearing in the
// submitted message up in the conversation area is ignored.
const SUBMIT_STUCK_RE = /^[^\S\n]*❯[^\S\n]*(?:\[Pasted text|<polygram-info|<session-context|<channel\b)/im;
  ...
  async function pasteAndEnter(name, text) {
    ...
    try {
      ...
      for (let attempt = 0; attempt <= submitCfg.retries; attempt += 1) {
        ...
        if (!SUBMIT_STUCK_RE.test(pane)) return res; // submitted ✓
