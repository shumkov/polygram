...
- **Unix-socket IPC per bot.** `/tmp/polygram-<bot>.sock`. Cron jobs and external scripts call the bot to send messages, so the transcript is unified and the bot is the only writer to its own DB.
