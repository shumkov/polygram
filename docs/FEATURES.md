...
- **Codex app-server (native macOS arm64 / Linux x64 beta).** Optional ChatGPT-backed Codex sessions use the target-specific pinned app-server directly, with mid-turn steering and durable stop/recovery fences. Other host targets, including Windows, are rejected. The Codex child uses an owner-only validated temp directory without changing Claude child environments. Command network and model-native web search are disabled, product MCP tools and interactive approvals are unavailable, and long-running commands must remain in the foreground. Detached/background servers are unsupported and may survive hard runtime loss.
